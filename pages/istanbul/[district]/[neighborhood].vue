...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/istanbul.json')
)                                                                                                                  ;
...
return istanbulData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/İstanbul)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `İstanbul ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu İstanbul ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}
...
text: `İstanbul, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/istanbul" class="hover:text-slate-900 transition-colors">İSTANBUL</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/istanbul/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu İstanbul {{
...
<p class="text-slate-600">İstanbul {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, İstanbul ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                           ;</p>
<ol>
...
<NuxtLink to="/istanbul" class="mt-6 inline-block text-red-700 underline">İstanbul Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
