...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const filter = ref("");
    const { data: muglaData } = ([__temp, __restore] = withAsyncContext(async () => useAsyncData("mugla-specific-data", async () => {
      const response = await $fetch("https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/mugla.json");
      const cityObj = {
        name: "MU\u011ELA",
        districts: {}
      };
    ...
    const districtItem = computed(() => {
      var _a;
      return (_a = muglaData.value) == null ? void 0 : _a.districts[distSlug.value];
    });
    const pageTitle = computed(() => districtItem.value ? `${titleCase(districtItem.value.name)} Posta Kodlar\u0131 - Mu\u011Fla` : "\u0130l\xE7e Bulunamad\u0131");
    const pageDesc = computed(() => districtItem.value ? `Mu\u011Fla ${titleCase(districtItem.value.name)} il\xE7esine ba\u011Fl\u0131 mahallelerin posta kodlar\u0131n\u0131 g\xF6rmek i\xE7in t\u0131klay\u0131n!` : "Posta Kodu Rehberi");
    useHead({
      title: pageTitle,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isValid.value) {
        ...
        _push(`<div><h1 class="text-3xl font-bold text-slate-900 tracking-tight">${ssrInterpolate(("titleCase" in _ctx ? _ctx.titleCase : unref(titleCase))(districtItem.value.name))} Posta Kodlar\u0131</h1><p class="text-slate-500 text-sm font-medium uppercase tracking-wider">MU\u011ELA</p></div></div><div class="relative w-full md:w-64">`);
...
//# sourceMappingURL=index-B2z_Ge6m.mjs.map
