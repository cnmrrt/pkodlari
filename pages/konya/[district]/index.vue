...
const { data: konyaData } = await useAsyncData<CityData>('konya-specific-data', async () => {
const response = await $fetch<any[]>('https://pkodlari.com/data/konya.json');
...
const districtItem = computed(() => konyaData.value?.districts[distSlug.value]) ;

const pageTitle = computed(() => districtItem.value ? `${titleCase(districtItem.value.name)} Posta Kodları - Balıkesir` : 'İlçe Bulunamadı');
const pageDesc = computed(() => districtItem.value ? `Balıkesir ${titleCase(districtItem.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
<NuxtLink to="/konya" class="text-slate-400 hover:text-slate-900 transition-colors"><ArrowLeft class="w-5 h-5" /></NuxtLink>
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(districtItem.name) }} Posta Kodları</h1>
<p class="text-slate-500 text-sm font-medium uppercase tracking-wider">BALIKESİR</p>
</div>
</div>
