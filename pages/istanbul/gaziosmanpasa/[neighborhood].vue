...
const { data: districtData } = await useAsyncData("gaziosmanpasa-details", () =>
  $fetch<any[]>("https://pkodlari.com/data/istanbul/gaziosmanpasa.json")
);

const cityName = "İstanbul";
const citySlug = "istanbul";
const districtName = "Gaziosmanpaşa";
const districtSlug = "gaziosmanpasa";
...
const pageDesc = computed(() =>
  neighData.value
    ? `${cityName} ilinin ${currentDistrictName.value} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu öğrenmek için tıklayın!`
    : "Posta Kodu Rehberi"
);
...
const share = () => {
  if (navigator.share && neighData.value) {
    navigator.share({
      ...
      text: `${cityName}, ${currentDistrictName.value}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<template>
  <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
    <nav class="breadcrumb">
      ...
      <NuxtLink :to="`/${citySlug}`" class="hover:text-slate-900 transition-colors">{{ cityName }}</NuxtLink>
      <ChevronRight class="w-3 h-3" />
      <NuxtLink :to="`/${citySlug}/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ currentDistrictName }}</NuxtLink>
    </nav>
    ...
    <div class="main-info-container">
      ...
      <div class="post-code-view-container">
        <div class="sub-post-code-view-container">
          ...
          <button @click="copyToClipboard" class="copy-btn active:scale-95" :class="copied ? 'green-btn' : 'black-btn'">
            <Check v-if="copied"/>
            <Copy v-else />
            {{ copied ? 'KOPYALANDI' : 'KODU KOPYALA' }}
          </button>
        </div>
        <div class="btn-bg">{{ String(neighData.postaKodu).substring(0, 2) }}</div>
      </div>
    </div>
...
<h2>Bu ilçedeki diğer mahalleler</h2>      <div class="neighborhoods-links-container">
        <NuxtLink v-for="neigh in otherNeighborhoods" :key="neigh.slug" :to="`/${citySlug}/${districtSlug}/${neigh.slug}`" >
          <div>
            <h3>{{ titleCase(neigh.name) }}</h3>
            <p>{{ neigh.zipCode }}</p>
          </div>
    ...
    <div class="page-text">
      ...
      <p>{{ neighData.postaKodu }} posta kodu {{ cityName }} {{ currentDistrictName }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
    </div>
    <div class="page-text">
      <h2>{{ currentDistrictName }} {{ titleCase(neighData.mahalle) }} Posta Kodu Rehberi</h2>
      <p>{{ cityName }} {{ currentDistrictName }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde kullanabilirsiniz.</p>
      <br />
    ...
    <div class="page-text">
      ...
      <p>{{ titleCase(neighData.mahalle) }}, {{ cityName }} ilinin {{ currentDistrictName }} ilçesine bağlıdır. Posta kodunun ilk 2 hanesi il plaka kodunu ({{ String(neighData.postaKodu).substring(0, 2) }}), son 3 hanesi ise ilçe ve mahalle dağıtım bölgesini temsil eder.</p>
    </div>
  </div>
  ...
  <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
    <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
      ...
      <NuxtLink :to="`/${districtSlug}`" class="mt-6 inline-block text-red-700 underline">{{ districtName }} Sayfasına Dön</NuxtLink>
    </div>
  </div>
</template>
