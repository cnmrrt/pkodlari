...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/artvin.json')
)                                                                                                                ;
...
return artvinData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Artvin ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Artvin, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/artvin" class="hover:text-slate-900 transition-colors">ARTVİN</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/artvin/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Artvin {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Artvin ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/artvin" class="mt-6 inline-block text-red-700 underline">Artvin Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
