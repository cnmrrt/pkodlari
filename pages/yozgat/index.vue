...
const pageTitle = computed(() => yozgatData.value ? `${titleCase(yozgatData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => yozgatData.value ? `${titleCase(yozgatData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(yozgatData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!yozgatData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(yozgatData.name) }} Posta Kodları</h1>
