...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const filter = ref("");
    const { data: edirneData } = ([__temp, __restore] = withAsyncContext(async () => useAsyncData("edirne-specific-data", async () => {
      const response = await $fetch("https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/edirne.json");
      const cityObj = {
        name: "ED\u0130RNE",
        districts: {}
      };
    ...
    const districtItem = computed(() => {
      var _a;
      return (_a = edirneData.value) == null ? void 0 : _a.districts[distSlug.value];
    });
    const pageTitle = computed(() => districtItem.value ? `${titleCase(districtItem.value.name)} Posta Kodlar\u0131 - Edirne` : "\u0130l\xE7e Bulunamad\u0131");
    const pageDesc = computed(() => districtItem.value ? `Edirne ${titleCase(districtItem.value.name)} il\xE7esine ba\u011Fl\u0131 mahallelerin posta kodlar\u0131n\u0131 g\xF6rmek i\xE7in t\u0131klay\u0131n!` : "Posta Kodu Rehberi");
    useHead({
      title: pageTitle,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isValid.value) {
        ...
        _push(`<div><h1 class="text-3xl font-bold text-slate-900 tracking-tight">${ssrInterpolate(("titleCase" in _ctx ? _ctx.titleCase : unref(titleCase))(districtItem.value.name))} Posta Kodlar\u0131</h1><p class="text-slate-500 text-sm font-medium uppercase tracking-wider">ED\u0130RNE</p></div></div><div class="relative w-full md:w-64">`);
        ...
        ssrRenderList(neighs.value, ([nSlug, nItem]) => {
          _push(ssrRenderComponent(_component_NuxtLink, {
            key: nSlug,
            to: `/edirne/${distSlug.value}/${nSlug}`,
...
//# sourceMappingURL=index-CVYkCVwA.mjs.map
