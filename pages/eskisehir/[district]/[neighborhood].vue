...
const { data: eskisehirData } = await useAsyncData('eskisehir-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/eskisehir.json')
)                                                                                                               ;
...
return eskisehirData.value.find(item =>
...
text: `Eskişehir, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/eskisehir/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<NuxtLink to="/eskisehir" class="mt-6 inline-block text-red-700 underline">Eskişehir Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
