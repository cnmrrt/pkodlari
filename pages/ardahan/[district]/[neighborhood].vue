...
const { data: ardahanData } = await useAsyncData("ardahan-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/ardahan.json")
);
...
const pageDesc = computed(() =>
  neighData.value
    ? `Ardahan ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(
...
const share = () => {
  if (navigator.share && neighData.value) {
    navigator.share({
      ...
      text: `Ardahan, ${titleCase(neighData.value.ilce)}, ${titleCase(
        neighData.value.mahalle
...
<template>
  <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
    ...
    >
      ...
      <NuxtLink to="/ardahan" class="hover:text-slate-900 transition-colors"
        >ARDAHAN</NuxtLink
      >
      <ChevronRight class="w-3 h-3" />
      <NuxtLink
        :to="`/ardahan/${districtSlug}`"
    ...
    <div class="main-info-container">
      <div class="main-info-title-container">
        ...
        <p class="main-info-subtitle">
          {{ titleCase(neighData.ilce) }}, Ardahan
        </p>
      </div>
    ...
    <div class="page-text">
      ...
      <p>
        {{ neighData.postaKodu }} posta kodu Ardahan {{ titleCase(neighData.ilce) }}
    ...
    <div class="page-text">
      ...
      <p>
        Ardahan {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait
    ...
    <div class="page-text">
      ...
      <p>
        {{ titleCase(neighData.mahalle) }}, Ardahan ilinin
  ...
  <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
    <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
      ...
      <NuxtLink to="/ardahan" class="mt-6 inline-block text-red-700 underline"
        >Ardahan Sayfasına Dön</NuxtLink
      >
    </div>
  </div>
</template>
