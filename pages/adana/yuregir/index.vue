...
const { data: rawNeighborhoods } = await useAsyncData("yuregir-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/adana/yuregir.json")
);

const cityName = "Adana";
const citySlug = "adana";
const districtName = "Yüreğir";
  ...
  <div v-if="isValid" class="animate-in fade-in duration-500 max-w-4xl mx-auto">
    <div class="header">
      ...
      <div class="neighborhood-search-container">
        <Search />
        <input v-model="filter" type="text" placeholder="Mahallelerde ara..." />
      </div>
    </div>

    <div class="neighborhoods-links-container">
      <NuxtLink v-for="nItem in neighs" :key="nItem.slug" :to="`/${citySlug}/${districtSlug}/${nItem.slug}`"
