...
const { data: kocaeliData } = await useAsyncData<CityData>('kocaeli-specific-data', async () => {
...
const districtItem = computed(() => kocaeliData.value?.districts[distSlug.value])              ;
...
const pageDesc = computed(() => districtItem.value && neighItem.value ? `Kocaeli ilinin ${titleCase(districtItem.value.name)} ilçesine bağlı ${titleCase(neighItem.value.name)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
"position": 2,
"name": "Kocaeli",
"item": `https://postakodu.com/kocaeli`
},
{
"@type": "ListItem",
"position": 3,
"name": titleCase(districtItem.value.name),
"item": `https://postakodu.com/kocaeli/${distSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighItem.value.name),
"item": `https://postakodu.com/kocaeli/${distSlug.value}/${neighborhoodSlug.value}`
}
]
})
}
...
text: `Kocaeli, ${titleCase(districtItem.value.name)}, ${titleCase(neighItem.value.name)} mahallesinin posta kodu: ${neighItem.value.zipCode}`,
...
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/kocaeli/${distSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(districtItem.name) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(districtItem.name) }}, KOCAELİ</p>
</div>
...
<div class="mt-8 text-center mb-8">
    ...
    <p class="text-slate-600">{{ neighItem.zipCode }} posta kodu Kocaeli {{ titleCase(districtItem.name) }} {{ titleCase(neighItem.name) }}'ne aittir.</p>
</div>
</div>
