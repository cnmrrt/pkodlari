...
const pageTitle = computed(() => ispartaData.value ? `${titleCase(ispartaData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => ispartaData.value ? `${titleCase(ispartaData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(ispartaData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                             ;

const isValid = computed(() => !!ispartaData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(ispartaData.name) }} Posta Kodları</h1>
