...
const pageTitle = computed(() => giresunData.value ? `${titleCase(giresunData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => giresunData.value ? `${titleCase(giresunData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(giresunData.name) }} Posta Kodları</h1>
