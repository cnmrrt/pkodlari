...
const pageTitle = `Bolu Posta Kodları ve İlçeleri ${currentYear}`;
...
<template>
    <div class="animate-in fade-in duration-500 max-w-4xl mx-auto">
        <div class="cityTitleContainer">
            ...
            <div>
                <h1>Bolu Posta Kodları</h1>
                <p>{{ districts.length }} İLÇE</p>
            </div>
        </div>

        <div class="neighborhoods-links-container">
            <NuxtLink v-for="([distSlug, distItem]) in districts" :key="distSlug" :to="`/bolu/${distSlug}`">
                <div>
