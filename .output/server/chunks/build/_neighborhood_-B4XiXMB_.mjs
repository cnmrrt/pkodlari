...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: ankaraData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "ankara-data",
      () => $fetch("https://pkodlari.com/data/ankara/all.json")
    ...
    const pageTitle = computed(
      ...
      )}/Ankara)` : "Posta Kodu Bulunamad\u0131"
    );
    const pageDesc = computed(
      () => neighData.value ? `Ankara ilinin ${titleCase(neighData.value.ilce)} il\xE7esine ba\u011Fl\u0131 ${titleCase(
            ...
            children: JSON.stringify({
              ...
              itemListElement: [
                ...
                },
                {
                  ...
                  item: `https://pkodlari.com/ankara/${districtSlug.value}`
                },
                {
                  "@type": "ListItem",
                  position: 4,
                  name: titleCase(neighData.value.mahalle),
                  item: `https://pkodlari.com/ankara/${districtSlug.value}/${neighborhoodSlug.value}`
                }
              ]
            })
          };
              ...
              mainEntity: [
                {
                  ...
                  acceptedAnswer: {
                    "@type": "Answer",
                    text: `${neighData.value.postaKodu} posta kodu Ankara ${titleCase(
                      neighData.value.ilce
    ...
    const otherNeighborhoods = computed(() => {
      if (!ankaraData.value || !neighData.value) return [];
      return ankaraData.value.filter((item) => slugify(item.ilce || "MERKEZ") === districtSlug.value).map((item) => ({
        name: item.mahalle,
      ...
      if (isValid.value) {
        ...
        if (otherNeighborhoods.value.length) {
          ...
          ssrRenderList(otherNeighborhoods.value, (neigh) => {
            _push(ssrRenderComponent(_component_NuxtLink, {
              key: neigh.slug,
              to: `/ankara/${districtSlug.value}/${neigh.slug}`,
...
//# sourceMappingURL=_neighborhood_-B4XiXMB_.mjs.map
