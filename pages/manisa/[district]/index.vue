...
const pageTitle = computed(() => districtData.value ? `Manisa ${titleCase(districtData.value.name)} Posta Kodları` : 'İlçe Bulunamadı');
const pageDesc = computed(() => districtData.value ? `Manisa ilinin ${titleCase(districtData.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
<NuxtLink to="/manisa" class="text-slate-400 hover:text-slate-900 transition-colors"><ArrowLeft class="w-5 h-5" /></NuxtLink>
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(districtData.name) }} Posta Kodları</h1>
<p class="text-slate-500 text-sm font-medium uppercase tracking-wider">Manisa</p>
</div>
</div>
...
:key="nItem.slug"
:to="`/manisa/${districtSlug}/${nItem.slug}`"
...
<p>İlçe bulunamadı.</p>
<NuxtLink to="/manisa" class="text-slate-900 underline mt-4 inline-block">Manisa sayfasına dön</NuxtLink>
</div>
</template>
