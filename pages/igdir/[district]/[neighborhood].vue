...
const { data: igdirData } = await useAsyncData('igdir-data', () =>
$fetch<any[]>('https://pkodlari.com/data/igdir.json')
)                                                                                                                  ;
...
if (!igdirData.value) return null                     ;
return igdirData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Iğdır)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Iğdır ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"position": 2,
"name": "Iğdır",
"item": "https://pkodlari.com/igdir"
},
{
"@type": "ListItem",
"position": 3,
"name": titleCase(neighData.value.ilce),
"item": `https://pkodlari.com/igdir/${districtSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighData.value.mahalle),
"item": `https://pkodlari.com/igdir/${districtSlug.value}/${neighborhoodSlug.value}`
}
]
})
}                                                                                                                                                ;
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Iğdır ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}                                                                                                                                                ;
...
text: `Iğdır, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/igdir" class="hover:text-slate-900 transition-colors">KIRŞEHİR</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/igdir/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Iğdır</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Iğdır {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
...
<p class="text-slate-600">Iğdır {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde hata payını sıfıra indirmek için bu kodu kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Iğdır ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde ;</p>
...
<NuxtLink to="/igdir" class="mt-6 inline-block text-red-700 underline">Iğdır Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
