...
const { data: kilisData } = await useAsyncData('kilis-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/kilis.json')
)                                                                                                               ;
...
return kilisData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Kilis ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Kilis, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/kilis" class="hover:text-slate-900 transition-colors">KİLİS</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/kilis/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Kilis {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Kilis ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                               ;</p>
<ol>
...
<NuxtLink to="/kilis" class="mt-6 inline-block text-red-700 underline">Kilis Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
