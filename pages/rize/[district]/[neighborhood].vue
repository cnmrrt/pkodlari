...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/rize.json')
)                                                                                                              ;
...
if (!rizeData.value) return null                         ;
return rizeData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Rize)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Rize ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu öğrenmek için tıklayın!` : 'Posta Kodu Rehberi');
...
{ "@type": "ListItem", "position": 3, "name": titleCase(neighData.value.ilce), "item": `https://pkodlari.com/rize/${districtSlug.value}` },
{ "@type": "ListItem", "position": 4, "name": titleCase(neighData.value.mahalle), "item": `https://pkodlari.com/rize/${districtSlug.value}/${neighborhoodSlug.value}` }
]
})
}
}),
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Rize ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}
...
text: `Rize, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/rize" class="hover:text-slate-900 transition-colors">RİZE</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/rize/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Rize</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Rize {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
<div class="mt-8 text-center mb-8">
...
<p class="text-slate-600">Rize {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Rize ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodunun ilk 2 hanesi il plaka kodunu, son 3 hanesi ise ilçe ve mahalle dağıtım bölgesini temsil eder.</p>
</div>
</div>
...
<NuxtLink to="/rize" class="mt-6 inline-block text-red-700 underline">Rize Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
