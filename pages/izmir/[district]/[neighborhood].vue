...
const { data: izmirData } = await useAsyncData("izmir-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/izmir/all.json")
);
...
const pageDesc = computed(() =>
  neighData.value
    ? `İzmir ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(
...
const share = () => {
  if (navigator.share && neighData.value) {
    navigator.share({
      ...
      text: `İzmir, ${titleCase(neighData.value.ilce)}, ${titleCase(
        neighData.value.mahalle
...
<template>
  <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
    ...
    >
      ...
      <NuxtLink to="/izmir" class="hover:text-slate-900 transition-colors"
        >İZMİR</NuxtLink
      >
      <ChevronRight class="w-3 h-3" />
      <NuxtLink
        :to="`/izmir/${districtSlug}`"
    ...
    <div class="main-info-container">
      <div class="main-info-title-container">
        ...
        <p class="main-info-subtitle">
          {{ titleCase(neighData.ilce) }}, İzmir
        </p>
      </div>
...
<h2>Bu ilçedeki diğer mahalleler</h2>      <div class="neighborhoods-links-container">
        <NuxtLink
          ...
          :key="neigh.slug"
          :to="`/izmir/${districtSlug}/${neigh.slug}`"
    ...
    <div class="page-text">
      ...
      <p>
        {{ neighData.postaKodu }} posta kodu İzmir {{ titleCase(neighData.ilce) }}
    ...
    <div class="page-text">
      ...
      <p>
        İzmir {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait
    ...
    <div class="page-text">
      ...
      <p>
        {{ titleCase(neighData.mahalle) }}, İzmir ilinin
  ...
  <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
    <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
      ...
      <NuxtLink to="/izmir" class="mt-6 inline-block text-red-700 underline"
        >İzmir Sayfasına Dön</NuxtLink
      >
    </div>
  </div>
</template>
