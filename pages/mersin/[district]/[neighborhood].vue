...
return mersinData.value.find(item =>
...
text: `Mersin, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/mersin" class="hover:text-slate-900 transition-colors">MERSİN</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/mersin/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Mersin {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Mersin ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/mersin" class="mt-6 inline-block text-red-700 underline">Mersin Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
