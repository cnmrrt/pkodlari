...
<template>
  <div v-if="isValid" class="animate-in fade-in duration-500 max-w-2xl mx-auto">
    ...
    >
      ...
      <NuxtLink to="/adiyaman" class="hover:text-slate-900 transition-colors"
        >ADIYAMAN</NuxtLink
      >
      <ChevronRight class="w-3 h-3" />
      <NuxtLink
        :to="`/adiyaman/${districtSlug}`"
  ...
  <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
    <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
      ...
      <NuxtLink to="/adiyaman" class="mt-6 inline-block text-red-700 underline"
        >Adıyaman Sayfasına Dön</NuxtLink
      >
    </div>
  </div>
</template>
