...
const districtName = "Alucra";
const districtSlug = "alucra";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: giresunAlucraData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "giresun-alucra-data",
      () => $fetch(useRequestURL().origin + "/data/giresun/alucra.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!giresunAlucraData.value) return null;
      return giresunAlucraData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!giresunAlucraData.value || !neighData.value) return [];
      return giresunAlucraData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/giresun/alucra/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-S3kahvKL.mjs.map
