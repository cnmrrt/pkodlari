...
const neighData = computed(() => {
    ...
    return zonguldakData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Zonguldak)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Zonguldak ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu öğrenmek için tıklayın!` : 'Posta Kodu Rehberi');
            ...
            return {
                ...
                children: JSON.stringify({
                    ...
                    "itemListElement": [
                        ...
                        { "@type": "ListItem", "position": 2, "name": "Zonguldak", "item": "https://pkodlari.com/zonguldak" },
                        { "@type": "ListItem", "position": 3, "name": titleCase(neighData.value.ilce), "item": `https://pkodlari.com/zonguldak/${districtSlug.value}` },
                        { "@type": "ListItem", "position": 4, "name": titleCase(neighData.value.mahalle), "item": `https://pkodlari.com/zonguldak/${districtSlug.value}/${neighborhoodSlug.value}` }
                    ]
                })
            }
        }),
            ...
            return {
                ...
                children: JSON.stringify({
                    ...
                    "mainEntity": [{
                        ...
                        "acceptedAnswer": {
                            "@type": "Answer",
                            "text": `${neighData.value.postaKodu} posta kodu Zonguldak ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
                        }
                    }]
                })
            }
...
const share = () => {
    if (navigator.share && neighData.value) {
        navigator.share({
            ...
            text: `Zonguldak, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<template>
    <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
        <nav class="breadcrumb">
            ...
            <ChevronRight class="w-3 h-3" />
            <NuxtLink :to="`/zonguldak/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
        ...
        <div class="main-info-container">
            <div class="main-info-title-container">
                ...
                <p class="main-info-subtitle">{{ titleCase(neighData.ilce) }}, Zonguldak</p>
            </div>
        ...
        <div class="page-text">
            ...
            <p>{{ neighData.postaKodu }} posta kodu Zonguldak {{ titleCase(neighData.ilce) }} {{
                titleCase(neighData.mahalle) }}'ne aittir.</p>
        </div>
        <div class="page-text">
            <h2>{{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }} Posta Kodu Rehberi</h2>
            <p>Zonguldak {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu
                bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde
                kullanabilirsiniz.</p>
            <br>
        ...
        <div class="page-text">
            ...
            <ul>
                <li><b>Sıralama Hızı:</b> PTT ve özel kargo şirketlerinin otomatik ayrıştırma makineleri gönderinizi
                    posta koduna göre sınıflandırır.</li>
                <li><b>Yanlış Teslimat Önleme:</b> Doğru kod, gönderinizin başka bir şehirdeki adaş mahalleye gitmesini
                    engeller.</li>
        ...
        <div class="page-text">
            ...
            <p>{{ titleCase(neighData.mahalle) }}, Zonguldak ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır.
                Posta kodunun ilk 2 hanesi il plaka kodunu, son 3 hanesi ise ilçe ve mahalle dağıtım bölgesini temsil
                eder.</p>
        </div>
    </div>
    ...
    <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
        <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
            ...
            <NuxtLink to="/zonguldak" class="mt-6 inline-block text-red-700 underline">Zonguldak Sayfasına Dön
            </NuxtLink>
        </div>
    </div>
</template>
