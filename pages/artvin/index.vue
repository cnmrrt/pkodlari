...
const pageTitle = computed(() => artvinData.value ? `${titleCase(artvinData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => artvinData.value ? `${titleCase(artvinData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(artvinData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!artvinData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(artvinData.name) }} Posta Kodları</h1>
