...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/bolu.json')
)                                                                                                              ;
...
const pageDesc = computed(() => neighData.value ? `Bolu ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Bolu, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/bolu" class="hover:text-slate-900 transition-colors">BOLU</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/bolu/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Bolu {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Bolu ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                                ;</p>
<ol>
...
<NuxtLink to="/bolu" class="mt-6 inline-block text-red-700 underline">Bolu Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
