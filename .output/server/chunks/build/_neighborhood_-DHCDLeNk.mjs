...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: canakkaleData } = ([__temp, __restore] = withAsyncContext(async () => useAsyncData("canakkale-specific-data", async () => {
      const response = await $fetch("https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/canakkale.json");
      const cityObj = {
        name: "\xC7ANAKKALE",
        districts: {}
      };
    ...
    const districtItem = computed(() => {
      var _a;
      return (_a = canakkaleData.value) == null ? void 0 : _a.districts[distSlug.value];
    });
    ...
    const pageDesc = computed(() => districtItem.value && neighItem.value ? `\xC7anakkale ilinin ${titleCase(districtItem.value.name)} il\xE7esine ba\u011Fl\u0131 ${titleCase(neighItem.value.name)}'nin posta kodunu g\xF6rmek i\xE7in t\u0131klay\u0131n!` : "Posta Kodu Rehberi");
    useHead({
      title: pageTitle,
            ...
            children: JSON.stringify({
              ...
              "itemListElement": [
                ...
                },
                {
                  ...
                  "position": 2,
                  "name": "\xC7anakkale",
                  "item": `https://postakodu.com/canakkale`
                },
                {
                  "@type": "ListItem",
                  "position": 3,
                  "name": titleCase(districtItem.value.name),
                  "item": `https://postakodu.com/canakkale/${distSlug.value}`
                },
                {
                  "@type": "ListItem",
                  "position": 4,
                  "name": titleCase(neighItem.value.name),
                  "item": `https://postakodu.com/canakkale/${distSlug.value}/${neighborhoodSlug.value}`
                }
              ]
            })
          };
...
//# sourceMappingURL=_neighborhood_-DHCDLeNk.mjs.map
