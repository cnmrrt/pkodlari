...
const { data: samsunData } = await useAsyncData('samsun-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/samsun.json')
)                                                                                                                ;
...
return samsunData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Samsun ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Samsun, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/samsun" class="hover:text-slate-900 transition-colors">SAMSUN</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/samsun/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Samsun {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Samsun ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/samsun" class="mt-6 inline-block text-red-700 underline">Samsun Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
