...
const pageTitle = computed(() => districtItem.value ? `${titleCase(districtItem.value.name)} Posta Kodları - Kayseri` : 'İlçe Bulunamadı');
const pageDesc = computed(() => districtItem.value ? `Kayseri ${titleCase(districtItem.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
<p class="text-slate-500 text-sm font-medium uppercase tracking-wider">KAYSERİ</p>
</div>
</div>
