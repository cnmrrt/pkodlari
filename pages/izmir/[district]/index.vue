...
const pageTitle = computed(() => districtItem.value ? `${titleCase(districtItem.value.name)} Posta Kodları - İzmir` : 'İlçe Bulunamadı');
const pageDesc = computed(() => districtItem.value ? `İzmir ${titleCase(districtItem.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
<p class="text-slate-500 text-sm font-medium uppercase tracking-wider">İZMİR</p>
</div>
</div>
