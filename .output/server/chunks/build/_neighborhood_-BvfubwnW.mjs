...
const districtName = "G\xF6nen";
const districtSlug = "gonen";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: balikesirGonenData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "balikesir-gonen-data",
      () => $fetch("https://pkodlari.com/data/tr/balikesir/gonen.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!balikesirGonenData.value) return null;
      return balikesirGonenData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!balikesirGonenData.value || !neighData.value) return [];
      return balikesirGonenData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/balikesir/gonen/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-BvfubwnW.mjs.map
