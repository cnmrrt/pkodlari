...
const pageTitle = computed(() => izmirData.value ? `${titleCase(izmirData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => izmirData.value ? `${titleCase(izmirData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(izmirData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!izmirData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(izmirData.name) }} Posta Kodları</h1>
