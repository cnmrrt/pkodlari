...
useHead({
title: computed(() => districtItem.value ? `${titleCase(districtItem.value.name)} Posta Kodları - Edirne` : 'İlçe Bulunamadı'),
meta: [
{
name: 'description',
content: computed(() => {
if (!districtItem.value) return '';
return `Edirne ${titleCase(districtItem.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!`    ;
})
}
],
script: [
