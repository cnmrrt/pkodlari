...
const districtItem = computed(() => kayseriData.value?.districts[distSlug.value])              ;
...
const pageDesc = computed(() => districtItem.value && neighItem.value ? `Kayseri ilinin ${titleCase(districtItem.value.name)} ilçesine bağlı ${titleCase(neighItem.value.name)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
"item": `https://postakodu.com/kayseri/${distSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighItem.value.name),
"item": `https://postakodu.com/kayseri/${distSlug.value}/${neighborhoodSlug.value}`
}
]
})
}
...
text: `Kayseri, ${titleCase(districtItem.value.name)}, ${titleCase(neighItem.value.name)} mahallesinin posta kodu: ${neighItem.value.zipCode}`,
...
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/kayseri/${distSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(districtItem.name) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(districtItem.name) }}, KAYSERİ</p>
</div>
...
<div class="mt-8 text-center mb-8">
    ...
    <p class="text-slate-600">{{ neighItem.zipCode }} posta kodu Kayseri {{ titleCase(districtItem.name) }} {{ titleCase(neighItem.name) }}'ne aittir.</p>
</div>
</div>
