...
const districtName = "\u015Ei\u015Fli";
const districtSlug = "sisli";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: istanbulSisliData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "istanbul-sisli-data",
      () => $fetch("https://pkodlari.com/data/istanbul/sisli.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!istanbulSisliData.value) return null;
      return istanbulSisliData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!istanbulSisliData.value || !neighData.value) return [];
      return istanbulSisliData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/istanbul/sisli/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-DuFVxR7W.mjs.map
