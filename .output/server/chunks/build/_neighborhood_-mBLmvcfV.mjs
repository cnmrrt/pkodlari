...
const districtName = "Abana";
const districtSlug = "abana";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuAbanaData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-abana-data",
      () => $fetch(useRequestURL().origin + "/data/kastamonu/abana.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuAbanaData.value) return null;
      return kastamonuAbanaData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuAbanaData.value || !neighData.value) return [];
      return kastamonuAbanaData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/abana/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-mBLmvcfV.mjs.map
