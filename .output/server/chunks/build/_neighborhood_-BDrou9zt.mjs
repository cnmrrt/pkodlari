...
const districtName = "KADIK\xD6Y";
const districtSlug = "kadikoy";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: istanbulKadikoyData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "istanbul-kadikoy-data",
      () => $fetch("https://pkodlari.com/data/istanbul/kadikoy.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!istanbulKadikoyData.value) return null;
      return istanbulKadikoyData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!istanbulKadikoyData.value || !neighData.value) return [];
      return istanbulKadikoyData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/istanbul/kadikoy/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-BDrou9zt.mjs.map
