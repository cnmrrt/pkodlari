...
const { data: giresunAlucraData } = await useAsyncData("giresun-alucra-data", () =>
  $fetch<any[]>(useRequestURL().origin + "/data/giresun/alucra.json")
);
...
const otherNeighborhoods = computed(() => {
  ...
  return giresunAlucraData.value
