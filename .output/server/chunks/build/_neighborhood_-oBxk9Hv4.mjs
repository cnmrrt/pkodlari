...
const districtName = "Bozkurt";
const districtSlug = "bozkurt";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuBozkurtData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-bozkurt-data",
      () => $fetch(useRequestURL().origin + "/data/kastamonu/bozkurt.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuBozkurtData.value) return null;
      return kastamonuBozkurtData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuBozkurtData.value || !neighData.value) return [];
      return kastamonuBozkurtData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/bozkurt/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-oBxk9Hv4.mjs.map
