...
const { data: ispartaData } = await useAsyncData('isparta-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/isparta.json')
)                                                                                                               ;
...
return ispartaData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Isparta ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Isparta, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/isparta" class="hover:text-slate-900 transition-colors">ISPARTA</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/isparta/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Isparta {{
...
<p class="text-slate-600">Isparta {{
...
<p class="text-slate-600">Posta kodu, bir adresin en spesifik bileşenidir. {{ titleCase(neighData.mahalle) }} için tanımlanan {{ neighData.postaKodu }} numarasını kullanmak şu aispartatajları sağlar:</p>
<ul>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Isparta ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/isparta" class="mt-6 inline-block text-red-700 underline">Isparta Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
