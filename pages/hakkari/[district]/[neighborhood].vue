...
return hakkariData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Hakkari ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Hakkari, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/hakkari" class="hover:text-slate-900 transition-colors">HAKKARİ</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/hakkari/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p>{{ neighData.postaKodu }} posta kodu Hakkari {{
...
<p>Posta kodu, bir adresin en spesifik bileşenidir. {{ titleCase(neighData.mahalle) }} için tanımlanan {{ neighData.postaKodu }} numarasını kullanmak şu ahakkaritajları sağlar:</p>
<ul>
...
<p>{{ titleCase(neighData.mahalle) }}, Hakkari ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/hakkari" class="mt-6 inline-block text-red-700 underline">Hakkari Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
