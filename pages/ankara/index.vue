...
const pageTitle = computed(() => ankaraData.value ? `${titleCase(ankaraData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => ankaraData.value ? `${titleCase(ankaraData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(ankaraData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                            ;

const isValid = computed(() => !!ankaraData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(ankaraData.name) }} Posta Kodları</h1>
