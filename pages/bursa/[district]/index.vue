...
<NuxtLink to="/bursa" class="text-slate-400 hover:text-slate-900 transition-colors"><ArrowLeft class="w-5 h-5" /></NuxtLink>
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(districtData.name) }} Posta Kodları</h1>
<p class="text-slate-500 text-sm font-medium uppercase tracking-wider">Bursa</p>
</div>
</div>
...
:key="nItem.slug"
:to="`/bursa/${districtSlug}/${nItem.slug}`"
...
<p>İlçe bulunamadı.</p>
<NuxtLink to="/bursa" class="text-slate-900 underline mt-4 inline-block">Bursa sayfasına dön</NuxtLink>
</div>
</template>
