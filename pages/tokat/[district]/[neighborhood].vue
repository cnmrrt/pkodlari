...
const { data: tokatData } = await useAsyncData('tokat-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/tokat.json')
)                                                                                                               ;
...
return tokatData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Tokat ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Tokat, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/tokat" class="hover:text-slate-900 transition-colors">TOKAT</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/tokat/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Tokat {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Tokat ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/tokat" class="mt-6 inline-block text-red-700 underline">Tokat Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
