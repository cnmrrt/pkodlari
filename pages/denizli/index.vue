...
const pageTitle = computed(() => denizliData.value ? `${titleCase(denizliData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => denizliData.value ? `${titleCase(denizliData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(denizliData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!denizliData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(denizliData.name) }} Posta Kodları</h1>
