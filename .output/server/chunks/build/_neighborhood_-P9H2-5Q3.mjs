...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: istanbulData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "istanbul-data",
      () => $fetch("https://pkodlari.com/data/istanbul/all.json")
    ...
    const pageTitle = computed(
      ...
      )}/\u0130stanbul)` : "Posta Kodu Bulunamad\u0131"
    );
    const pageDesc = computed(
      () => neighData.value ? `\u0130stanbul ilinin ${titleCase(neighData.value.ilce)} il\xE7esine ba\u011Fl\u0131 ${titleCase(
    ...
    const otherNeighborhoods = computed(() => {
      ...
      return istanbulData.value.filter((item) => slugify(item.ilce || "MERKEZ") === districtSlug.value).map((item) => ({
        name: item.mahalle,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isValid.value) {
        ...
        _push(ssrRenderComponent(unref(ChevronRight), { class: "w-3 h-3" }, null, _parent));
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: `/istanbul/${districtSlug.value}`,
          class: "hover:text-slate-900 transition-colors"
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
        ...
        if (otherNeighborhoods.value.length) {
          ...
          ssrRenderList(otherNeighborhoods.value, (neigh) => {
            _push(ssrRenderComponent(_component_NuxtLink, {
              key: neigh.slug,
              to: `/istanbul/${districtSlug.value}/${neigh.slug}`,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/istanbul/[district]/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-P9H2-5Q3.mjs.map
