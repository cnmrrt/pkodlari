...
const { data: istanbulKadikoyData } = await useAsyncData("istanbul-kadikoy-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/istanbul/kadikoy.json")
);

const cityName = "İstanbul";
const citySlug = "istanbul";
const districtName = "KADIKÖY";
...
const pageTitle = computed(() =>
  neighData.value
    ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${
          ...
          mainEntity: [
            {
              ...
              acceptedAnswer: {
                "@type": "Answer",
                text: `${neighData.value.postaKodu} posta kodu ${cityName} ${
                  currentDistrictName.value
...
const otherNeighborhoods = computed(() => {
  ...
  return istanbulKadikoyData.value
