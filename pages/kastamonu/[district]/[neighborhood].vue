...
const { data: kastamonuData } = await useAsyncData('kastamonu-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/kastamonu.json')
)                                                                                                                   ;
...
return kastamonuData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Kastamonu ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Kastamonu, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/kastamonu" class="hover:text-slate-900 transition-colors">KASTAMONU</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/kastamonu/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Kastamonu {{
...
<p class="text-slate-600">Kastamonu {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Kastamonu ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                           ;</p>
<ol>
...
<NuxtLink to="/kastamonu" class="mt-6 inline-block text-red-700 underline">Kastamonu Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
