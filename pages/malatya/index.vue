...
const pageTitle = computed(() => malatyaData.value ? `${titleCase(malatyaData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => malatyaData.value ? `${titleCase(malatyaData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(malatyaData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!malatyaData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(malatyaData.name) }} Posta Kodları</h1>
