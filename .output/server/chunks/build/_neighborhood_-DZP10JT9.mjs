...
const citySlug = "adana";
const districtName = "Y\xFCre\u011Fir";
const districtSlug = "yuregir";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: adanaYuregirData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "adana-yuregir-data",
      () => $fetch("https://pkodlari.com/data/adana/yuregir.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!adanaYuregirData.value) return null;
      return adanaYuregirData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!adanaYuregirData.value || !neighData.value) return [];
      return adanaYuregirData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/adana/yuregir/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-DZP10JT9.mjs.map
