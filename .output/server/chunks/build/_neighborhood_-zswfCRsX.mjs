...
const districtName = "MAMAK";
const districtSlug = "mamak";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: AnkaraMamakData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "ankara-mamak-data",
      () => $fetch("https://pkodlari.com/data/ankara/mamak.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!AnkaraMamakData.value) return null;
      return AnkaraMamakData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!AnkaraMamakData.value || !neighData.value) return [];
      return AnkaraMamakData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/ankara/mamak/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-zswfCRsX.mjs.map
