...
const pageTitle = computed(() => bilecikData.value ? `${titleCase(bilecikData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => bilecikData.value ? `${titleCase(bilecikData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(bilecikData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!bilecikData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(bilecikData.name) }} Posta Kodları</h1>
