...
const districtName = "Devrekani";
const districtSlug = "devrekani";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuDevrekaniData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-devrekani-data",
      () => $fetch(useRequestURL().origin + "/data/kastamonu/devrekani.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuDevrekaniData.value) return null;
      return kastamonuDevrekaniData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuDevrekaniData.value || !neighData.value) return [];
      return kastamonuDevrekaniData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/devrekani/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-CTFkwpFQ.mjs.map
