...
const { data: musData } = await useAsyncData('mus-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/mus.json')
)                                                                                                               ;
...
return musData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Muş ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Muş, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/mus" class="hover:text-slate-900 transition-colors">MUŞ</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/mus/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Muş {{
...
<p class="text-slate-600">Posta kodu, bir adresin en spesifik bileşenidir. {{ titleCase(neighData.mahalle) }} için tanımlanan {{ neighData.postaKodu }} numarasını kullanmak şu amustajları sağlar:</p>
<ul>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Muş ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/mus" class="mt-6 inline-block text-red-700 underline">Muş Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
