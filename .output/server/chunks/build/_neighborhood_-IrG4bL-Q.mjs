...
const districtName = "Ta\u015Fk\xF6pr\xFC";
const districtSlug = "taskopru";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuTaskopruData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-taskopru-data",
      () => $fetch(useRequestURL().origin + "/data/kastamonu/taskopru.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuTaskopruData.value) return null;
      return kastamonuTaskopruData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuTaskopruData.value || !neighData.value) return [];
      return kastamonuTaskopruData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/taskopru/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-IrG4bL-Q.mjs.map
