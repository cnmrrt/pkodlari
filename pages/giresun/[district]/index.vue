...
const pageTitle = computed(() =>
  districtData.value
    ? `Giresun ${titleCase(districtData.value.name)} Posta Kodları`
    : "İlçe Bulunamadı"
);
const pageDesc = computed(() =>
  districtData.value
    ? `Giresun ilinin ${titleCase(
...
<template>
  ...
  <div v-else class="py-32 text-center text-slate-400">
    <p>İlçe bulunamadı.</p>
    <NuxtLink to="/giresun" class="text-slate-900 underline mt-4 inline-block"
      >Giresun sayfasına dön</NuxtLink
    >
  </div>
</template>
