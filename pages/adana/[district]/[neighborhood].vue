...
const { data: adanaData } = await useAsyncData("adana-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/adana/all.json")
);
...
const pageDesc = computed(() =>
  neighData.value
    ? `Adana ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(
...
const share = () => {
  if (navigator.share && neighData.value) {
    navigator.share({
      ...
      text: `Adana, ${titleCase(neighData.value.ilce)}, ${titleCase(
        neighData.value.mahalle
...
<template>
  <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
    <nav class="breadcrumb">
      ...
      <ChevronRight class="w-3 h-3" />
      <NuxtLink :to="`/adana/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
    ...
    <div class="main-info-container">
      <div class="main-info-title-container">
        ...
        <p class="main-info-subtitle">{{ titleCase(neighData.ilce) }}, Adana</p>
      </div>
    ...
    <div class="page-text mt-8 text-center mb-8">
      <h2>
        {{ neighData.postaKodu }} nerenin posta kodu?
      </h2>
      <p>
        {{ neighData.postaKodu }} posta kodu Adana {{ titleCase(neighData.ilce) }}
    ...
    <div class="page-text">
      ...
      <p>
        Adana {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait
      ...
      <br />
      <p>
        <strong>{{ titleCase(neighData.mahalle) }} Posta Kodu:
    ...
    <div class="page-text">
      ...
      <p>
        {{ titleCase(neighData.mahalle) }}, Adana ilinin
        {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodunun ilk 2 hanesi il
        plaka kodunu, son 3 hanesi ise ilçe ve mahalle dağıtım bölgesini temsil eder.
      </p>
    </div>
  </div>
