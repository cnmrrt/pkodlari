...
const cityName = "\xC7ankaya";
const citySlug = "cankaya";
const districtName = "\xC7ANKAYA";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: Ankara\u00C7ankayaData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "ankara-cankaya-data",
      () => $fetch("https://pkodlari.com/data/ankara/cankaya.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!Ankara\u00C7ankayaData.value) return null;
      return Ankara\u00C7ankayaData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!Ankara\u00C7ankayaData.value || !neighData.value) return [];
      return Ankara\u00C7ankayaData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/ankara/cankaya/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-ezPBl17e.mjs.map
