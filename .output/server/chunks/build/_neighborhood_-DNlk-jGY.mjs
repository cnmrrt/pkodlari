...
const districtName = "Ara\xE7";
const districtSlug = "arac";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuAracData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-arac-data",
      () => $fetch(useRequestURL().origin + "/data/kastamonu/arac.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuAracData.value) return null;
      return kastamonuAracData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuAracData.value || !neighData.value) return [];
      return kastamonuAracData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/arac/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-DNlk-jGY.mjs.map
