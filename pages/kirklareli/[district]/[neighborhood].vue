...
const { data: kirklareliData } = await useAsyncData('kirklareli-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/kirklareli.json')
)                                                                                                                    ;
...
if (!kirklareliData.value) return null                   ;
return kirklareliData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Kırklareli)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Kırklareli ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"position": 2,
"name": "Kırklareli",
"item": "https://pkodlari.com/kirklareli"
},
{
"@type": "ListItem",
"position": 3,
"name": titleCase(neighData.value.ilce),
"item": `https://pkodlari.com/kirklareli/${districtSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighData.value.mahalle),
"item": `https://pkodlari.com/kirklareli/${districtSlug.value}/${neighborhoodSlug.value}`
}
]
})
}                                                                                                                                                 ;
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Kırklareli ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}                                                                                                                                                 ;
...
text: `Kırklareli, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/kirklareli" class="hover:text-slate-900 transition-colors">KIRKLARELİ</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/kirklareli/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Kırklareli</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Kırklareli {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
...
<p class="text-slate-600">Kırklareli {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde hata payını sıfıra indirmek için bu kodu kullanabilirsiniz.</p>
<br>
...
<ul class="text-left max-w-lg mx-auto space-y-2 text-slate-600">
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Kırklareli ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde ;</p>
<ol class="text-left max-w-lg mx-auto space-y-2 text-slate-600 list-decimal pl-5 mt-4">
...
<NuxtLink to="/kirklareli" class="mt-6 inline-block text-red-700 underline">Kırklareli Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
