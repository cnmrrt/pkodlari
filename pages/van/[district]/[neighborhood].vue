...
return vanData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Van ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Van, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/van" class="hover:text-slate-900 transition-colors">VAN</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/van/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Van {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Van ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/van" class="mt-6 inline-block text-red-700 underline">Van Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
