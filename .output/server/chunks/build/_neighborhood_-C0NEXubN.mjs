...
const districtName = "\u0130nebolu";
const districtSlug = "inebolu";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuIneboluData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-inebolu-data",
      () => $fetch(useRequestURL().origin + "/data/kastamonu/inebolu.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuIneboluData.value) return null;
      return kastamonuIneboluData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuIneboluData.value || !neighData.value) return [];
      return kastamonuIneboluData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/inebolu/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-C0NEXubN.mjs.map
