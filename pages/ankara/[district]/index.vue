...
useHead({
title: computed(() => districtItem.value ? `${titleCase(districtItem.value.name)} Posta Kodları - Ankara` : 'İlçe Bulunamadı'),
meta: [
{
name: 'description',
content: computed(() => {
if (!districtItem.value) return '';
return `Ankara ${titleCase(districtItem.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!`    ;
})
}
],
script: [
