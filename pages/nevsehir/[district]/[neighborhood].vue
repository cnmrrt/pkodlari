...
const { data: nevsehirData } = await useAsyncData('nevsehir-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/nevsehir.json')
)                                                                                                                  ;
...
if (!nevsehirData.value) return null                     ;
return nevsehirData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Nevşehir)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Nevşehir ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"position": 2,
"name": "Nevşehir",
"item": "https://pkodlari.com/nevsehir"
},
{
"@type": "ListItem",
"position": 3,
"name": titleCase(neighData.value.ilce),
"item": `https://pkodlari.com/nevsehir/${districtSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighData.value.mahalle),
"item": `https://pkodlari.com/nevsehir/${districtSlug.value}/${neighborhoodSlug.value}`
}
]
})
}                                                                                                                                               ;
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Nevşehir ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}                                                                                                                                               ;
...
text: `Nevşehir, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/nevsehir" class="hover:text-slate-900 transition-colors">NEVŞEHİR</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/nevsehir/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Nevşehir</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Nevşehir {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
...
<p class="text-slate-600">Nevşehir {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde hata payını sıfıra indirmek için bu kodu kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Nevşehir ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde ;</p>
...
<NuxtLink to="/nevsehir" class="mt-6 inline-block text-red-700 underline">Nevşehir Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
