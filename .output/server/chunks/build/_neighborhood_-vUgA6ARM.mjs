...
const districtName = "Bergama";
const districtSlug = "bergama";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: izmirBergamaData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "izmir-bergama-data",
      () => $fetch(useRequestURL().origin + "/data/izmir/bergama.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!izmirBergamaData.value) return null;
      return izmirBergamaData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!izmirBergamaData.value || !neighData.value) return [];
      return izmirBergamaData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/izmir/bergama/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-vUgA6ARM.mjs.map
