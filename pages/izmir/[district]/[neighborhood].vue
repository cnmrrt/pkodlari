...
if (!izmirData.value) return null                        ;
return izmirData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/İzmir)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `İzmir ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu öğrenmek için tıklayın!` : 'Posta Kodu Rehberi');
...
{ "@type": "ListItem", "position": 2, "name": "İzmir", "item": "https://pkodlari.com/izmir" },
{ "@type": "ListItem", "position": 3, "name": titleCase(neighData.value.ilce), "item": `https://pkodlari.com/izmir/${districtSlug.value}` },
{ "@type": "ListItem", "position": 4, "name": titleCase(neighData.value.mahalle), "item": `https://pkodlari.com/izmir/${districtSlug.value}/${neighborhoodSlug.value}` }
]
})
}
}),
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu İzmir ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}
...
text: `İzmir, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/izmir" class="hover:text-slate-900 transition-colors">İZMİR</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/izmir/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, İzmir</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu İzmir {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
<div class="mt-8 text-center mb-8">
...
<p class="text-slate-600">İzmir {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, İzmir ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                    ;</p>
<ol>
...
<NuxtLink to="/izmir" class="mt-6 inline-block text-red-700 underline">İzmir Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
