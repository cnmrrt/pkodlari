...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/aksaray.json')
)                                                                                                               ;
...
return aksarayData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Aksaray ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Aksaray, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/aksaray" class="hover:text-slate-900 transition-colors">AKSARAY</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/aksaray/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Aksaray {{
...
<p class="text-slate-600">Aksaray {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Aksaray ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/aksaray" class="mt-6 inline-block text-red-700 underline">Aksaray Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
