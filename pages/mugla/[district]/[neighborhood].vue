...
return muglaData.value.find(item =>
...
text: `Muğla, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/mugla" class="hover:text-slate-900 transition-colors">MUĞLA</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/mugla/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Muğla {{
...
<p class="text-slate-600">Posta kodu, bir adresin en spesifik bileşenidir. {{ titleCase(neighData.mahalle) }} için tanımlanan {{ neighData.postaKodu }} numarasını kullanmak şu amuglatajları sağlar:</p>
<ul>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Muğla ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/mugla" class="mt-6 inline-block text-red-700 underline">Muğla Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
