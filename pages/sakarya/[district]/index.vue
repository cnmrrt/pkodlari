...
const pageTitle = computed(() => districtData.value ? `Sakarya ${titleCase(districtData.value.name)} Posta Kodları` : 'İlçe Bulunamadı');
...
<template>
    <div v-if="isValid" class="animate-in fade-in duration-500 max-w-4xl mx-auto">
        <div class="header">
            <div class="districtTitleContainer">
                ...
                <div>
                    <h1>{{ titleCase(districtData.name) }}
                        Posta Kodları</h1>
                    <p>Sakarya</p>
                </div>
            </div>
            <div class="neighborhood-search-container">
                <Search />
                <input type="text" placeholder="Mahallelerde ara..."
        ...
        <div class="neighborhoods-links-container">
            <NuxtLink v-for="nItem in neighs" :key="nItem.slug" :to="`/sakarya/${districtSlug}/${nItem.slug}`"
                >
    ...
    <div v-else class="py-32 text-center text-slate-400">
        <p>İlçe bulunamadı.</p>
        <NuxtLink to="/sakarya" class="text-slate-900 underline mt-4 inline-block">Sakarya sayfasına dön</NuxtLink>
    </div>
</template>
