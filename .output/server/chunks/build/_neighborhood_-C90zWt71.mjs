...
const districtName = "Espiye";
const districtSlug = "espiye";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: giresunEspiyeData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "giresun-espiye-data",
      () => $fetch("https://pkodlari.com/data/giresun/espiye.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!giresunEspiyeData.value) return null;
      return giresunEspiyeData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!giresunEspiyeData.value || !neighData.value) return [];
      return giresunEspiyeData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/giresun/espiye/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-C90zWt71.mjs.map
