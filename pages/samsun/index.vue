...
const pageTitle = computed(() => samsunData.value ? `${titleCase(samsunData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => samsunData.value ? `${titleCase(samsunData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(samsunData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                            ;

const isValid = computed(() => !!samsunData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(samsunData.name) }} Posta Kodları</h1>
