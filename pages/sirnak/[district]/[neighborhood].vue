...
const { data: sirnakData } = await useAsyncData('sirnak-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/sirnak.json')
)                                                                                                                ;
...
return sirnakData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Şırnak ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Şırnak, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/sirnak" class="hover:text-slate-900 transition-colors">ŞIRNAK</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/sirnak/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Şırnak</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Şırnak {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
...
<p class="text-slate-600">Şırnak {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde hata payını sıfıra indirmek için bu kodu kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Şırnak ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde ;</p>
...
<NuxtLink to="/sirnak" class="mt-6 inline-block text-red-700 underline">Şırnak Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
