...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/aydin.json')
)                                                                                                               ;
...
return aydinData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Aydın ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Aydın, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/aydin" class="hover:text-slate-900 transition-colors">AYDIN</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/aydin/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Aydın {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Aydın ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/aydin" class="mt-6 inline-block text-red-700 underline">Aydın Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
