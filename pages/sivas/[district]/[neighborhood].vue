...
const { data: sivasData } = await useAsyncData("sivas-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/sivas/all.json")
);
...
const pageTitle = computed(() =>
  neighData.value
    ? `Sivas ${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(
...
const pageDesc = computed(() =>
  neighData.value
    ? `Sivas ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(
...
const share = () => {
  if (navigator.share && neighData.value) {
    navigator.share({
      ...
      text: `Sivas, ${titleCase(neighData.value.ilce)}, ${titleCase(
        neighData.value.mahalle
...
<template>
  <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
    ...
    >
      ...
      <NuxtLink to="/sivas" class="hover:text-slate-900 transition-colors"
        >SİVAS</NuxtLink
      >
      <ChevronRight class="w-3 h-3" />
      <NuxtLink
        :to="`/sivas/${districtSlug}`"
    ...
    <div class="main-info-container">
      <div class="main-info-title-container">
        ...
        <p class="main-info-subtitle">
          {{ titleCase(neighData.ilce) }}, Sivas
        </p>
      </div>
    ...
    <div class="page-text">
      ...
      <p>
        {{ neighData.postaKodu }} posta kodu Sivas {{ titleCase(neighData.ilce) }}
    ...
    <div class="page-text">
      ...
      <p>
        Sivas {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait
    ...
    <div class="page-text">
      ...
      <p>
        {{ titleCase(neighData.mahalle) }}, Sivas ilinin
        {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı
        incelendiğinde ;
      </p>
      <ol
        class="text-left max-w-lg mx-auto space-y-2 text-slate-600 list-decimal pl-5 mt-4"
      >
  ...
  <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
    <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
      ...
      <NuxtLink to="/sivas" class="mt-6 inline-block text-red-700 underline"
        >Sivas Sayfasına Dön</NuxtLink
      >
    </div>
  </div>
</template>
