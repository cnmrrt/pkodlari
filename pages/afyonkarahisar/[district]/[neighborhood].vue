...
const { data: afyonData } = await useAsyncData('afyonkarahisar-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/afyonkarahisar.json')
)                                                                                                                        ;
...
if (!afyonData.value) return null                        ;
return afyonData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Afyonkarahisar)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Afyonkarahisar ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu öğrenmek için tıklayın!` : 'Posta Kodu Rehberi');
...
{ "@type": "ListItem", "position": 2, "name": "Afyonkarahisar", "item": "https://pkodlari.com/afyonkarahisar" },
{ "@type": "ListItem", "position": 3, "name": titleCase(neighData.value.ilce), "item": `https://pkodlari.com/afyonkarahisar/${districtSlug.value}` },
{ "@type": "ListItem", "position": 4, "name": titleCase(neighData.value.mahalle), "item": `https://pkodlari.com/afyonkarahisar/${districtSlug.value}/${neighborhoodSlug.value}` }
]
})
}
}),
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Afyonkarahisar ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}
...
text: `Afyonkarahisar, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/afyonkarahisar" class="hover:text-slate-900 transition-colors">AFYONKARAHİSAR</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/afyonkarahisar/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Afyonkarahisar</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Afyonkarahisar {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
<div class="mt-8 text-center mb-8">
...
<p class="text-slate-600">Afyonkarahisar {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Afyonkarahisar ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodunun ilk 2 hanesi il plaka kodunu, son 3 hanesi ise ilçe ve mahalle dağıtım bölgesini temsil eder.</p>
</div>
</div>
...
<NuxtLink to="/afyonkarahisar" class="mt-6 inline-block text-red-700 underline">Afyonkarahisar Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
