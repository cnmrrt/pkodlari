...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/kocaeli.json')
)                                                                                                                 ;
...
if (!kocaeliData.value) return null                      ;
return kocaeliData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Kocaeli)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Kocaeli ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"item": `https://pkodlari.com/kocaeli/${districtSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighData.value.mahalle),
"item": `https://pkodlari.com/kocaeli/${districtSlug.value}/${neighborhoodSlug.value}`
}
]
})
}                                                                                                                                             ;
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Kocaeli ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}                                                                                                                                             ;
...
text: `Kocaeli, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/kocaeli" class="hover:text-slate-900 transition-colors">KOCAELİ</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/kocaeli/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Kocaeli</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Kocaeli {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.`</p>
</div>
...
<p class="text-slate-600">Kocaeli {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde hata payını sıfıra indirmek için bu kodu kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Kocaeli ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde ;</p>
...
<NuxtLink to="/kocaeli" class="mt-6 inline-block text-red-700 underline">Kocaeli Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
