...
const districtItem = computed(() => zonguldakData.value?.districts[distSlug.value])              ;
...
const pageDesc = computed(() => districtItem.value && neighItem.value ? `Zonguldak ilinin ${titleCase(districtItem.value.name)} ilçesine bağlı ${titleCase(neighItem.value.name)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
"item": `https://postakodu.com/zonguldak/${distSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighItem.value.name),
"item": `https://postakodu.com/zonguldak/${distSlug.value}/${neighborhoodSlug.value}`
}
]
})
}
...
text: `Zonguldak, ${titleCase(districtItem.value.name)}, ${titleCase(neighItem.value.name)} mahallesinin posta kodu: ${neighItem.value.zipCode}`,
...
<div class="mt-8 text-center mb-8">
    ...
    <p class="text-slate-600">{{ neighItem.zipCode }} posta kodu Zonguldak {{ titleCase(districtItem.name) }} {{ titleCase(neighItem.name) }}'ne aittir.</p>
</div>
</div>
