...
const cityName = "Artvin";
const citySlug = "artvin";
const districtName = "Yusufeli";
const districtSlug = "yusufeli";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: artvinYusufeliData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "artvin-yusufeli-data",
      () => $fetch("https://pkodlari.com/data/tr/artvin/yusufeli.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!artvinYusufeliData.value) return null;
      return artvinYusufeliData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!artvinYusufeliData.value || !neighData.value) return [];
      return artvinYusufeliData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/artvin/yusufeli/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_--iN0_YIC.mjs.map
