...
const pageTitle = computed(() => kocaeliData.value ? `${titleCase(kocaeliData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => kocaeliData.value ? `${titleCase(kocaeliData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(kocaeliData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!kocaeliData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(kocaeliData.name) }} Posta Kodları</h1>
