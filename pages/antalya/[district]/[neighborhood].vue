...
return antalyaData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Antalya ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Antalya, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/antalya" class="hover:text-slate-900 transition-colors">ANTALYA</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/antalya/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p>{{ neighData.postaKodu }} posta kodu Antalya {{
...
<p>Posta kodu, bir adresin en spesifik bileşenidir. {{ titleCase(neighData.mahalle) }} için tanımlanan {{ neighData.postaKodu }} numarasını kullanmak şu aantalyatajları sağlar:</p>
<ul>
...
<p>{{ titleCase(neighData.mahalle) }}, Antalya ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/antalya" class="mt-6 inline-block text-red-700 underline">Antalya Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
