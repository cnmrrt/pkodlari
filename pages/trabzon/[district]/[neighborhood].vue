...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/trabzon.json')
)                                                                                                               ;
...
return trabzonData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Trabzon ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Trabzon, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/trabzon" class="hover:text-slate-900 transition-colors">TRABZON</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/trabzon/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Trabzon {{
...
<p class="text-slate-600">Trabzon {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Trabzon ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/trabzon" class="mt-6 inline-block text-red-700 underline">Trabzon Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
