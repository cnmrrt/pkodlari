...
const districtName = "Koyulhisar";
const districtSlug = "koyulhisar";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: sivasKoyulhisarData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "sivas-koyulhisar-data",
      () => $fetch("https://pkodlari.com/data/sivas/koyulhisar.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!sivasKoyulhisarData.value) return null;
      return sivasKoyulhisarData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!sivasKoyulhisarData.value || !neighData.value) return [];
      return sivasKoyulhisarData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/sivas/koyulhisar/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-CnNHvrRM.mjs.map
