...
const { data: istanbulSariyerData } = await useAsyncData("istanbul-sariyer-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/istanbul/sariyer.json")
);
...
const pageTitle = computed(() =>
  neighData.value
    ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${currentDistrictName.value
    }/${cityName})`
          ...
          mainEntity: [
            {
              ...
              acceptedAnswer: {
                "@type": "Answer",
                text: `${neighData.value.postaKodu} posta kodu ${cityName} ${currentDistrictName.value
...
const otherNeighborhoods = computed(() => {
  ...
  return istanbulSariyerData.value
...
<h2>Bu ilçedeki diğer mahalleler</h2>      <div class="neighborhoods-links-container">
    ...
    <div class="page-text">
      ...
      <p>
        ...
        {{ currentDistrictName }} ilçesine bağlıdır. Posta kodunun ilk 2 hanesi il plaka
        kodunu ({{ String(neighData.postaKodu).substring(0, 2) }}), son 3 hanesi ise ilçe
        ve mahalle dağıtım bölgesini temsil eder.
      </p>
    </div>
  </div>
  ...
  <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
    <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
      ...
      <NuxtLink :to="`/${districtSlug}`" class="mt-6 inline-block text-red-700 underline">{{ districtName }} Sayfasına
        Dön</NuxtLink>
    </div>
  </div>
</template>
