...
const pageTitle = computed(() => gumushaneData.value ? `${titleCase(gumushaneData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => gumushaneData.value ? `${titleCase(gumushaneData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(gumushaneData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!gumushaneData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(gumushaneData.name) }} Posta Kodları</h1>
