...
const pageTitle = computed(() => bayburtData.value ? `${titleCase(bayburtData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => bayburtData.value ? `${titleCase(bayburtData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
if (!bayburtData.value) return []                                                                              ;
return Object.entries(bayburtData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!bayburtData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(bayburtData.name) }} Posta Kodları</h1>
