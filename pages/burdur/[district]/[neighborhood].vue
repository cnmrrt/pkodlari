...
const { data: burdurData } = await useAsyncData('burdur-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/burdur.json')
)                                                                                                                ;
...
if (!burdurData.value) return null                       ;
return burdurData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Burdur)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Burdur ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"position": 2,
"name": "Burdur",
"item": "https://pkodlari.com/burdur"
},
{
"@type": "ListItem",
"position": 3,
"name": titleCase(neighData.value.ilce),
"item": `https://pkodlari.com/burdur/${districtSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighData.value.mahalle),
"item": `https://pkodlari.com/burdur/${districtSlug.value}/${neighborhoodSlug.value}`
}
]
})
}                                                                                                                                            ;
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Burdur ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}                                                                                                                                            ;
...
text: `Burdur, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/burdur" class="hover:text-slate-900 transition-colors">BURDUR</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/burdur/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Burdur</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Burdur {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
...
<p class="text-slate-600">Burdur {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde hata payını sıfıra indirmek için bu kodu kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Burdur ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde ;</p>
...
<NuxtLink to="/burdur" class="mt-6 inline-block text-red-700 underline">Burdur Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
