...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/kayseri.json')
)                                                                                                               ;
...
return kayseriData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Kayseri ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Kayseri, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/kayseri" class="hover:text-slate-900 transition-colors">KAYSERİ</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/kayseri/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Kayseri {{
...
<p class="text-slate-600">Kayseri {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Kayseri ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/kayseri" class="mt-6 inline-block text-red-700 underline">Kayseri Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
