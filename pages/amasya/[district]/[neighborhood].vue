...
const neighData = computed(() => {
    ...
    return amasyaData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Amasya ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
const otherNeighborhoods = computed(() => {
    ...
    return amasyaData.value
        .filter((item) => slugify(item.ilce || titleCase(neighData.ilce)) === districtSlug)
        .map((item) => ({
            name: item.mahalle,
...
const share = () => {
    if (navigator.share && neighData.value) {
        navigator.share({
            ...
            text: `Amasya, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<template>
    <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
        <nav class="breadcrumb">
            <a href="/" class="hover:text-slate-900 transition-colors">TÜRKİYE</a><svg
                xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                class="lucide lucide-chevron-right-icon w-3 h-3">
                <path d="m9 18 6-6-6-6"></path>
            </svg><a href="/amasya" class="hover:text-slate-900 transition-colors">AMASYA</a><svg
                xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                class="lucide lucide-chevron-right-icon w-3 h-3">
                <path d="m9 18 6-6-6-6"></path>
            </svg><a :href="`/amasya/${districtSlug}/`" class="hover:text-slate-900 transition-colors">{{
                titleCase(neighData.ilce)
                }}</a>
        </nav>
        ...
        <div class="main-info-container">
            <div class="main-info-title-container">
                ...
                <h1>{{ titleCase(neighData.mahalle) }}
                </h1>
            ...
            <div class="post-code-view-container">
                <div class="sub-post-code-view-container">
                    ...
                    <button @click="copyToClipboard" class="copy-btn active:scale-95"
        ...
        <div v-if="otherNeighborhoods.length" class="other-neighborhoods">
            ...
            <div class="neighborhoods-links-container">
                <NuxtLink v-for="neigh in otherNeighborhoods" :key="neigh.slug"
        ...
        <div class="page-text">
            ...
            <p>{{ neighData.postaKodu }} posta kodu Amasya {{
        ...
        <div class="page-text">
            ...
            <p>Amasya {{
                titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri
                aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde hata payını sıfıra
                indirmek için bu kodu kullanabilirsiniz.</p>
            <br>
            <p><strong>{{ titleCase(neighData.mahalle) }} Posta Kodu: {{ neighData.postaKodu }}</strong></p>
        </div>
        <div class="page-text">
            <h2>Adres Yazımında Posta Kodunun Önemi</h2>
            <p>Posta kodu, bir adresin en spesifik bileşenidir. {{ titleCase(neighData.mahalle)
            }} için tanımlanan {{ neighData.postaKodu }} numarasını kullanmak şu avantajları sağlar:</p>
            <ul>
                <li><b>Sıralama Hızı:</b> PTT ve özel kargo şirketlerinin otomatik ayrıştırma makineleri, gönderinizi
                    adresten önce posta koduna göre sınıflandırır.</li>
                <li><b>Yanlış Teslimat Önleme:</b> Türkiye genelinde aynı ismi taşıyan yüzlerce mahalle bulunmaktadır.
                    Doğru kod, gönderinizin başka bir şehirdeki adaş mahalleye gitmesini engeller.</li>
                <li><b>Dijital Doğruluk:</b> Bankacılık ve e-devlet sistemlerinde adres teyidi yapılırken sistemler
                    genellikle bu kodu baz alır.</li>
            </ul>
        </div>
        <div class="page-text">
            <h2>Konum Ve Bölge Bilgileri</h2>
            <p>{{ titleCase(neighData.mahalle) }}, Amasya ilinin {{ titleCase(neighData.ilce) }}
                ilçesine bağlıdır. Posta kodu yapısı incelendiğinde ;</p>
            <ol>
    ...
    <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
        <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
            ...
            <NuxtLink to="/amasya" class="mt-6 inline-block text-red-700 underline">Amasya Sayfasına Dön</NuxtLink>
        </div>
    </div>
</template>
