...
const { data: yozgatData } = await useAsyncData('yozgat-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/yozgat.json')
)                                                                                                               ;
...
return yozgatData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Yozgat ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Yozgat, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/yozgat" class="hover:text-slate-900 transition-colors">YOZGAT</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/yozgat/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Yozgat {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Yozgat ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/yozgat" class="mt-6 inline-block text-red-700 underline">Yozgat Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
