...
const districtName = "SARIYER";
const districtSlug = "sariyer";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: istanbulSariyerData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "istanbul-sariyer-data",
      () => $fetch("https://pkodlari.com/data/istanbul/sariyer.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!istanbulSariyerData.value) return null;
      return istanbulSariyerData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!istanbulSariyerData.value || !neighData.value) return [];
      return istanbulSariyerData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/istanbul/sariyer/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-eta4GTm-.mjs.map
