...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/gaziantep.json')
)                                                                                                               ;
...
return gaziantepData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Gaziantep)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Gaziantep ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Gaziantep ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}                                                                                                                                            ;
...
text: `Gaziantep, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/gaziantep" class="hover:text-slate-900 transition-colors">GAZİANTEP</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/gaziantep/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Gaziantep {{
...
<p class="text-slate-600">Gaziantep {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Gaziantep ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/gaziantep" class="mt-6 inline-block text-red-700 underline">Gaziantep Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
