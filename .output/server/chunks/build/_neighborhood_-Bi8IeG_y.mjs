...
const cityName = "Rize";
const citySlug = "rize";
const districtName = "\xC7AYEL\u0130";
const districtSlug = "cayeli";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: RizeCayeliData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "rize-cayeli-data",
      () => $fetch("https://pkodlari.com/data/rize/cayeli.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!RizeCayeliData.value) return null;
      return RizeCayeliData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!RizeCayeliData.value || !neighData.value) return [];
      return RizeCayeliData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/rize/cayeli/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-Bi8IeG_y.mjs.map
