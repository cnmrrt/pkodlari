...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: izmirData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "izmir-data",
      () => $fetch("https://pkodlari.com/data/izmir/all.json")
    ...
    const pageTitle = computed(
      ...
      )}/\u0130zmir)` : "Posta Kodu Bulunamad\u0131"
    );
    const pageDesc = computed(
      () => neighData.value ? `\u0130zmir ilinin ${titleCase(neighData.value.ilce)} il\xE7esine ba\u011Fl\u0131 ${titleCase(
            ...
            children: JSON.stringify({
              ...
              itemListElement: [
                ...
                },
                {
                  ...
                  item: `https://pkodlari.com/izmir/${districtSlug.value}`
                },
                {
                  "@type": "ListItem",
                  position: 4,
                  name: titleCase(neighData.value.mahalle),
                  item: `https://pkodlari.com/izmir/${districtSlug.value}/${neighborhoodSlug.value}`
                }
              ]
            })
          };
              ...
              mainEntity: [
                {
                  ...
                  acceptedAnswer: {
                    "@type": "Answer",
                    text: `${neighData.value.postaKodu} posta kodu \u0130zmir ${titleCase(
                      neighData.value.ilce
    ...
    const otherNeighborhoods = computed(() => {
      if (!izmirData.value || !neighData.value) return [];
      return izmirData.value.filter((item) => slugify(item.ilce || "MERKEZ") === districtSlug.value).map((item) => ({
        name: item.mahalle,
      ...
      if (isValid.value) {
        ...
        if (otherNeighborhoods.value.length) {
          ...
          ssrRenderList(otherNeighborhoods.value, (neigh) => {
            _push(ssrRenderComponent(_component_NuxtLink, {
              key: neigh.slug,
              to: `/izmir/${districtSlug.value}/${neigh.slug}`,
...
//# sourceMappingURL=_neighborhood_-DsNQFi17.mjs.map
