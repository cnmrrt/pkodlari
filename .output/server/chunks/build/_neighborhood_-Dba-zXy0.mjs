...
import { ChevronRight, Check, Copy, MapPin } from 'lucide-vue-next';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: adanaData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "adana-data",
      () => $fetch("https://pkodlari.com/data/adana/all.json")
    ...
    const pageTitle = computed(
      () => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(
        neighData.value.ilce
      )}/Adana)` : "Posta Kodu Bulunamad\u0131"
    );
    const pageDesc = computed(
      () => neighData.value ? `Adana ilinin ${titleCase(neighData.value.ilce)} il\xE7esine ba\u011Fl\u0131 ${titleCase(
        neighData.value.mahalle
      )}'nin posta kodunu \xF6\u011Frenmek i\xE7in t\u0131klay\u0131n!` : "Posta Kodu Rehberi"
    );
    useHead({
      title: pageTitle,
            ...
            children: JSON.stringify({
              ...
              itemListElement: [
                ...
                },
                {
                  ...
                  item: `https://pkodlari.com/adana/${districtSlug.value}`
                },
                {
                  "@type": "ListItem",
                  position: 4,
                  name: titleCase(neighData.value.mahalle),
                  item: `https://pkodlari.com/adana/${districtSlug.value}/${neighborhoodSlug.value}`
                }
              ]
            })
          };
              ...
              mainEntity: [
                {
                  ...
                  acceptedAnswer: {
                    "@type": "Answer",
                    text: `${neighData.value.postaKodu} posta kodu Adana ${titleCase(
                      neighData.value.ilce
    ...
    const otherNeighborhoods = computed(() => {
      if (!adanaData.value || !neighData.value) return [];
      return adanaData.value.filter((item) => slugify(item.ilce || "MERKEZ") === districtSlug.value).map((item) => ({
        name: item.mahalle,
      ...
      if (isValid.value) {
        ...
        if (otherNeighborhoods.value.length) {
          ...
          ssrRenderList(otherNeighborhoods.value, (neigh) => {
            _push(`<a${ssrRenderAttr("to", `/adana/${districtSlug.value}/${neigh.slug}`)}><div><h3>${ssrInterpolate(unref(titleCase)(neigh.name))}</h3><p>${ssrInterpolate(neigh.zipCode)}</p></div><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"></path></svg></a>`);
          });
          _push(`<!--]--></div></div>`);
        } else {
          _push(`<!---->`);
        ...
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`Adana Sayfas\u0131na D\xF6n`);
            } else {
              return [
                createTextVNode("Adana Sayfas\u0131na D\xF6n")
              ];
            }
          }),
          _: 1
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/adana/[district]/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-Dba-zXy0.mjs.map
