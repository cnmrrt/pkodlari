<script setup lang="ts">
import { ArrowLeft, ChevronRight } from 'lucide-vue-next';
import { slugify, titleCase } from '~/utils/slugify';

const { data: madridData } = await useAsyncData('madrid-data', () =>
    $fetch<any[]>('https://pkodlari.com/data/es/madrid.json')
);

const districts = computed(() => {
    if (!madridData.value) return [];
    const districtMap: Record<string, { name: string, count: number }> = {};

    madridData.value.forEach(item => {
        const distName = item.ilce || 'MERKEZ';
        const distSlug = slugify(distName);
        if (!districtMap[distSlug]) {
            districtMap[distSlug] = { name: distName, count: 0 };
        }
        districtMap[distSlug].count++;
    });

    return Object.entries(districtMap).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
});

const currentYear = new Date().getFullYear();
const pageTitle = `Códigos postales y distritos de Madrid ${currentYear}`;
const pageDesc = "¡Haz clic aquí para ver los códigos postales de los distritos y barrios de la provincia de Madrid!";

useHead({
    htmlAttrs: {
        lang: 'es'
    },
    title: pageTitle,
    meta: [{ name: 'description', content: pageDesc }],
    script: [
        {
            type: 'application/ld+json',
            children: JSON.stringify({
                "@context": "https://schema.org",
                "@type": "BreadcrumbList",
                "itemListElement": [
                    {
                        "@type": "ListItem",
                        "position": 1,
                        "name": "Anasayfa",
                        "item": "https://pkodlari.com/es"
                    },
                    {
                        "@type": "ListItem",
                        "position": 2,
                        "name": "Madrid",
                        "item": "https://pkodlari.com/es/madrid"
                    }
                ]
            })
        }
    ]
});

usePageSeo({ title: pageTitle, description: pageDesc });
</script>

<template>
    <div class="animate-in fade-in duration-500 max-w-4xl mx-auto">
        <div class="cityTitleContainer">
            <NuxtLink to="/es">
                <ArrowLeft />
            </NuxtLink>
            <div>
                <h1>Códigos postales de Madrid</h1>
                <p>{{ districts.length }} DISTRITO</p>
            </div>
        </div>

        <div class="districts">
            <NuxtLink v-for="([distSlug, distItem]) in districts" :key="distSlug" :to="`/es/madrid/${distSlug}`">
                <div>
                    <h3>{{ titleCase(distItem.name) }}</h3>
                    <p>{{ distItem.count }} vecindario</p>
                </div>
                <ChevronRight class="w-5 h-5 text-slate-300" />
            </NuxtLink>
        </div>
    </div>
</template>
