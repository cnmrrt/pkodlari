...
const districtName = "G\xF6rele";
const districtSlug = "gorele";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: giresunGoreleData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "giresun-gorele-data",
      () => $fetch("https://pkodlari.com/data/giresun/gorele.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!giresunGoreleData.value) return null;
      return giresunGoreleData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!giresunGoreleData.value || !neighData.value) return [];
      return giresunGoreleData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/giresun/gorele/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-feodJ-UC.mjs.map
