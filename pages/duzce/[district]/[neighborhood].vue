...
const { data: duzceData } = await useAsyncData('duzce-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/duzce.json')
)                                                                                                               ;
...
return duzceData.value.find(item =>
...
text: `Düzce, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/duzce" class="hover:text-slate-900 transition-colors">DÜZCE</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/duzce/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Düzce {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Düzce ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/duzce" class="mt-6 inline-block text-red-700 underline">Düzce Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
