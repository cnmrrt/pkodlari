...
const cityName = "\u0130zmir";
const citySlug = "izmir";
const districtName = "Bornova";
const districtSlug = "bornova";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: istanbulSileData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "istanbul-sile-data",
      () => $fetch("https://pkodlari.com/data/izmir/bornova.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!istanbulSileData.value) return null;
      return istanbulSileData.value.find(
    ...
    const pageDesc = computed(
      () => neighData.value ? `${cityName} ilinin ${currentDistrictName.value} il\xE7esine ba\u011Fl\u0131 ${titleCase(
    ...
    const otherNeighborhoods = computed(() => {
      if (!istanbulSileData.value || !neighData.value) return [];
      return istanbulSileData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/izmir/bornova/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-UALdCw8U.mjs.map
