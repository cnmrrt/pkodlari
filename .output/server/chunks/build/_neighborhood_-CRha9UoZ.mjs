...
const citySlug = "giresun";
const districtName = "Bulancak";
const districtSlug = "bulancak";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: giresunBulancakData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "giresun-bulancak-data",
      () => $fetch(useRequestURL().origin + "/data/giresun/bulancak.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!giresunBulancakData.value) return null;
      return giresunBulancakData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!giresunBulancakData.value || !neighData.value) return [];
      return giresunBulancakData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/giresun/bulancak/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-CRha9UoZ.mjs.map
