...
return bartinData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Bartın ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Bartın, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/bartin" class="hover:text-slate-900 transition-colors">BARTIN</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/bartin/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Bartın {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Bartın ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/bartin" class="mt-6 inline-block text-red-700 underline">Bartın Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
