...
const pageTitle = computed(() => districtData.value ? `Malatya ${titleCase(districtData.value.name)} Posta Kodları` : 'İlçe Bulunamadı');
const pageDesc = computed(() => districtData.value ? `Malatya ilinin ${titleCase(districtData.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
<NuxtLink to="/malatya" class="text-slate-400 hover:text-slate-900 transition-colors">
...
:key="nItem.slug"
:to="`/malatya/${districtSlug}/${nItem.slug}`"
...
<p>İlçe bulunamadı.</p>
<NuxtLink to="/malatya" class="text-slate-900 underline mt-4 inline-block">Malatya sayfasına dön</NuxtLink>
</div>
</template>
