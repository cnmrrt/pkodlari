...
const { data: kastamonuIneboluData } = await useAsyncData("kastamonu-inebolu-data", () =>
  $fetch<any[]>(useRequestURL().origin + "/data/kastamonu/inebolu.json")
);
...
const otherNeighborhoods = computed(() => {
  ...
  return kastamonuIneboluData.value
