...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const districtData = computed(() => {
      ...
      const filtered = sirnakData.value.filter((item) => slugify(item.ilce || "MERKEZ") === districtSlug.value);
    ...
    const pageTitle = computed(() => districtData.value ? `Şırnak ${titleCase(districtData.value.name)} Posta Kodları` : "İlçe Bulunamadı");
    const pageDesc = computed(() => districtData.value ? `Şırnak ilinin ${titleCase(districtData.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!` : "Posta Kodu Rehberi");
    useHead({
      title: pageTitle,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isValid.value) {
        ...
        _push(`<div><h1 class="text-3xl font-bold text-slate-900 tracking-tight">${ssrInterpolate(unref(titleCase)(districtData.value.name))} Posta Kodları</h1><p class="text-slate-500 text-sm font-medium uppercase tracking-wider">Şırnak</p></div></div><div class="relative w-full md:w-64">`);
        ...
        ssrRenderList(neighs.value, (nItem) => {
          _push(ssrRenderComponent(_component_NuxtLink, {
            key: nItem.slug,
            to: `/sirnak/${districtSlug.value}/${nItem.slug}`,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/sirnak/[district]/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=index-Dd5VG2d3.mjs.map
