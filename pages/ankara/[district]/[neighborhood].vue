...
const pageDesc = computed(() =>
  neighData.value
    ? `Ankara ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(
...
const share = () => {
  if (navigator.share && neighData.value) {
    navigator.share({
      ...
      text: `Ankara, ${titleCase(neighData.value.ilce)}, ${titleCase(
        neighData.value.mahalle
...
<template>
  <div v-if="isValid" class="animate-in fade-in duration-500 max-w-2xl mx-auto">
    ...
    <div class="bg-white border border-slate-200 rounded-[2rem] p-8 md:p-12 shadow-sm">
      <div class="text-center mb-10">
        ...
        <p class="text-slate-500 font-medium uppercase tracking-widest text-sm">
          {{ titleCase(neighData.ilce) }}, Ankara
        </p>
      </div>
    ...
    <div v-if="otherNeighborhoods.length" class="mt-10">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <NuxtLink
          ...
          :key="neigh.slug"
          :to="`/ankara/${districtSlug}/${neigh.slug}`"
    ...
    <div class="mt-8 text-center mb-8">
      ...
      <p class="text-slate-600">
        {{ neighData.postaKodu }} posta kodu Ankara {{ titleCase(neighData.ilce) }}
    ...
    <div class="mt-8 text-center mb-8">
      ...
      <p class="text-slate-600">
        Ankara {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait
    ...
    <div class="mt-8 text-center mb-8">
      ...
      <p class="text-slate-600">
        {{ titleCase(neighData.mahalle) }}, Ankara ilinin
  ...
  <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
    <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
      ...
      <NuxtLink to="/ankara" class="mt-6 inline-block text-red-700 underline"
        >Ankara Sayfasına Dön</NuxtLink
      >
    </div>
  </div>
</template>
