...
const pageTitle = computed(() => diyarbakirData.value ? `${titleCase(diyarbakirData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => diyarbakirData.value ? `${titleCase(diyarbakirData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
if (!diyarbakirData.value) return []                                                                              ;
return Object.entries(diyarbakirData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!diyarbakirData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(diyarbakirData.name) }} Posta Kodları</h1>
