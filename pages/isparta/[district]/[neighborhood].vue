...
const { data: ispartaData } = await useAsyncData<CityData>('isparta-specific-data', async () => {
const response = await $fetch<any[]>('/api/data/isparta');
...
const districtItem = computed(() => ispartaData.value?.districts[distSlug.value])           ;
...
const pageDesc = computed(() => districtItem.value && neighItem.value ? `Isparta ilinin ${titleCase(districtItem.value.name)} ilçesine bağlı ${titleCase(neighItem.value.name)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
"position": 2,
"name": "Isparta",
"item": `https://postakodu.com/isparta`
},
{
"@type": "ListItem",
"position": 3,
"name": titleCase(districtItem.value.name),
"item": `https://postakodu.com/isparta/${distSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighItem.value.name),
"item": `https://postakodu.com/isparta/${distSlug.value}/${neighborhoodSlug.value}`
}
]
})
}
...
text: `Isparta, ${titleCase(districtItem.value.name)}, ${titleCase(neighItem.value.name)} mahallesinin posta kodu: ${neighItem.value.zipCode}`,
...
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/isparta/${distSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(districtItem.name) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(districtItem.name) }}, ISPARTA</p>
</div>
...
<p class="text-slate-600">{{ neighItem.zipCode }} posta kodu Isparta {{ titleCase(districtItem.name) }} {{ titleCase(neighItem.name) }}'ne aittir.</p>
</div>
</div>
