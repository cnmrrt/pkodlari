...
const districtName = "\xC7atalzeytin";
const districtSlug = "catalzeytin";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuCatalzeytinData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-catalzeytin-data",
      () => $fetch(useRequestURL().origin + "/data/kastamonu/catalzeytin.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuCatalzeytinData.value) return null;
      return kastamonuCatalzeytinData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuCatalzeytinData.value || !neighData.value) return [];
      return kastamonuCatalzeytinData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/catalzeytin/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-C_mi8LRH.mjs.map
