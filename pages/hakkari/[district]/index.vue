...
const pageTitle = computed(() => districtData.value ? `Hakkari ${titleCase(districtData.value.name)} Posta Kodları` : 'İlçe Bulunamadı');
const pageDesc = computed(() => districtData.value ? `Hakkari ilinin ${titleCase(districtData.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
<NuxtLink to="/hakkari" ><ArrowLeft/></NuxtLink>
<div>
...
:key="nItem.slug"
:to="`/hakkari/${districtSlug}/${nItem.slug}`"
...
<p>İlçe bulunamadı.</p>
<NuxtLink to="/hakkari" class="text-slate-900 underline mt-4 inline-block">Hakkari sayfasına dön</NuxtLink>
</div>
</template>
