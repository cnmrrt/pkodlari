...
const pageTitle = computed(() => boluData.value ? `${titleCase(boluData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => boluData.value ? `${titleCase(boluData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(boluData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!boluData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(boluData.name) }} Posta Kodları</h1>
