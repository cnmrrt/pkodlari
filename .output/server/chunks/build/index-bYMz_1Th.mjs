...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const pageDesc = computed(() => districtData.value ? `Karabük ilinin ${titleCase(districtData.value.name)} ilçesine bağlı mahallelerin posta kodlarını görmek için tıklayın!` : "Posta Kodu Rehberi");
    useHead({
      title: pageTitle,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isValid.value) {
        ...
        _push(`<div><h1 class="text-3xl font-bold text-slate-900 tracking-tight">${ssrInterpolate(unref(titleCase)(districtData.value.name))} Posta Kodları</h1><p class="text-slate-500 text-sm font-medium uppercase tracking-wider">Karabük</p></div></div><div class="relative w-full md:w-64">`);
