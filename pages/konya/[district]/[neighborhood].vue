...
const { data: konyaData } = await useAsyncData("konya-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/konya.json")
);
...
const pageDesc = computed(() =>
  neighData.value
    ? `Konya ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(
...
const share = () => {
  if (navigator.share && neighData.value) {
    navigator.share({
      ...
      text: `Konya, ${titleCase(neighData.value.ilce)}, ${titleCase(
        neighData.value.mahalle
...
<template>
  <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
    ...
    >
      ...
      <NuxtLink to="/konya" class="hover:text-slate-900 transition-colors"
        >KONYA</NuxtLink
      >
      <ChevronRight class="w-3 h-3" />
      <NuxtLink
        :to="`/konya/${districtSlug}`"
    ...
    <div class="main-info-container">
      <div class="main-info-title-container">
        ...
        <p class="main-info-subtitle">
          {{ titleCase(neighData.ilce) }}, Konya    
        </p>
      </div>
    ...
    <div class="page-text">
      ...
      <p>
        {{ neighData.postaKodu }} posta kodu Konya {{ titleCase(neighData.ilce) }}
    ...
    <div class="page-text">
      ...
      <p>
        Konya {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait
    ...
    <div class="page-text">
      ...
      <p>
        {{ titleCase(neighData.mahalle) }}, Konya ilinin
  ...
  <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
    <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
      ...
      <NuxtLink to="/konya" class="mt-6 inline-block text-red-700 underline"
        >Konya Sayfasına Dön</NuxtLink
      >
    </div>
  </div>
</template>
