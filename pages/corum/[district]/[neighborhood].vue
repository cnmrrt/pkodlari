...
if (!corumData.value) return null                        ;
return corumData.value.find(item =>
...
text: `Çorum, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/corum" class="hover:text-slate-900 transition-colors">ÇORUM</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/corum/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Çorum</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Çorum {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
<div class="mt-8 text-center mb-8">
...
<p class="text-slate-600">Çorum {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Çorum ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodunun ilk 2 hanesi il plaka kodunu, son 3 hanesi ise ilçe ve mahalle dağıtım bölgesini temsil eder.</p>
</div>
</div>
...
<NuxtLink to="/corum" class="mt-6 inline-block text-red-700 underline">Çorum Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
