...
const pageTitle = computed(() => amasyaData.value ? `${titleCase(amasyaData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => amasyaData.value ? `${titleCase(amasyaData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(amasyaData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                          ;

const isValid = computed(() => !!amasyaData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(amasyaData.name) }} Posta Kodları</h1>
