...
const { data: samsunData } = await useAsyncData<CityData>('samsun-specific-data', async () => {
const response = await $fetch<any[]>('/api/data/samsun');
...
const districtItem = computed(() => samsunData.value?.districts[distSlug.value])            ;
...
const pageDesc = computed(() => districtItem.value && neighItem.value ? `Samsun ilinin ${titleCase(districtItem.value.name)} ilçesine bağlı ${titleCase(neighItem.value.name)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
"position": 2,
"name": "Samsun",
"item": `https://postakodu.com/samsun`
},
{
"@type": "ListItem",
"position": 3,
"name": titleCase(districtItem.value.name),
"item": `https://postakodu.com/samsun/${distSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighItem.value.name),
"item": `https://postakodu.com/samsun/${distSlug.value}/${neighborhoodSlug.value}`
}
]
})
}
...
text: `Samsun, ${titleCase(districtItem.value.name)}, ${titleCase(neighItem.value.name)} mahallesinin posta kodu: ${neighItem.value.zipCode}`,
...
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/samsun/${distSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(districtItem.name) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(districtItem.name) }}, SAMSUN</p>
</div>
...
<p class="text-slate-600">{{ neighItem.zipCode }} posta kodu Samsun {{ titleCase(districtItem.name) }} {{ titleCase(neighItem.name) }}'ne aittir.</p>
</div>
</div>
