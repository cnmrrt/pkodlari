...
const districtName = "Azdavay";
const districtSlug = "azdavay";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuAzdavayData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-azdavay-data",
      () => $fetch("https://pkodlari.com/data/kastamonu/azdavay.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuAzdavayData.value) return null;
      return kastamonuAzdavayData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuAzdavayData.value || !neighData.value) return [];
      return kastamonuAzdavayData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/azdavay/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-Cr-i4BJ8.mjs.map
