...
const districtName = "Daday";
const districtSlug = "daday";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuDadayData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-daday-data",
      () => $fetch("https://pkodlari.com/data/kastamonu/daday.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuDadayData.value) return null;
      return kastamonuDadayData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuDadayData.value || !neighData.value) return [];
      return kastamonuDadayData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/daday/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-CJOQRMWx.mjs.map
