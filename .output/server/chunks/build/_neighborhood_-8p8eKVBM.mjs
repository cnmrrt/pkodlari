...
const districtName = "\xDCMRAN\u0130YE";
const districtSlug = "umraniye";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: istanbulUmraniyeData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "istanbul-umraniye-data",
      () => $fetch("https://pkodlari.com/data/istanbul/umraniye.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!istanbulUmraniyeData.value) return null;
      return istanbulUmraniyeData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!istanbulUmraniyeData.value || !neighData.value) return [];
      return istanbulUmraniyeData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/istanbul/umraniye/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-8p8eKVBM.mjs.map
