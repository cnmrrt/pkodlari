...
const districtName = "Dursunbey";
const districtSlug = "dursunbey";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: balikesirDursunbeyData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "balikesir-dursunbey-data",
      () => $fetch("https://pkodlari.com/data/balikesir/dursunbey.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!balikesirDursunbeyData.value) return null;
      return balikesirDursunbeyData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!balikesirDursunbeyData.value || !neighData.value) return [];
      return balikesirDursunbeyData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/balikesir/dursunbey/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-D18clwWj.mjs.map
