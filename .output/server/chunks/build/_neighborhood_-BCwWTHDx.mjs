...
const districtName = "Cide";
const districtSlug = "cide";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: kastamonuCideData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "kastamonu-cide-data",
      () => $fetch("https://pkodlari.com/data/kastamonu/cide.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!kastamonuCideData.value) return null;
      return kastamonuCideData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!kastamonuCideData.value || !neighData.value) return [];
      return kastamonuCideData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/kastamonu/cide/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-BCwWTHDx.mjs.map
