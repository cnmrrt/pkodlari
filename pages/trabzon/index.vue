...
const pageTitle = computed(() => trabzonData.value ? `${titleCase(trabzonData.value.name)} Posta Kodları` : 'Şehir Bulunamadı');
const pageDesc = computed(() => trabzonData.value ? `${titleCase(trabzonData.value.name)} iline bağlı ilçe ve mahallelerin posta kodlarını görmek için tıklayın!` : 'Posta Kodu Rehberi');
useHead({
title: pageTitle,
...
return Object.entries(trabzonData.value.districts).sort(([, a], [, b]) => a.name.localeCompare(b.name, 'tr'));
})                                                                                                             ;

const isValid = computed(() => !!trabzonData.value) ;
</script>
...
<div>
<h1 class="text-3xl font-bold text-slate-900 tracking-tight">{{ titleCase(trabzonData.name) }} Posta Kodları</h1>
