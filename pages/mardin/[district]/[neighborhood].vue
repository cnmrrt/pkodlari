...
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/mardin.json')
)                                                                                                               ;
...
return mardinData.value.find(item =>
...
text: `Mardin, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/mardin" class="hover:text-slate-900 transition-colors">MARDİN</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/mardin/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Mardin {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Mardin ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/mardin" class="mt-6 inline-block text-red-700 underline">Mardin Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
