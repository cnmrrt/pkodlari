...
const { data: sivasData } = await useAsyncData('sivas-data', () =>
$fetch<any[]>('https://pkodlari.com/data/sivas.json')
)                                                                                                                ;
...
if (!sivasData.value) return null                       ;
return sivasData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `Sivas ${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Sivas)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Sivas ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"position": 2,
"name": "Sivas",
"item": "https://pkodlari.com/sivas"
},
{
"@type": "ListItem",
"position": 3,
"name": titleCase(neighData.value.ilce),
"item": `https://pkodlari.com/sivas/${districtSlug.value}`
},
{
"@type": "ListItem",
"position": 4,
"name": titleCase(neighData.value.mahalle),
"item": `https://pkodlari.com/sivas/${districtSlug.value}/${neighborhoodSlug.value}`
}
]
})
}                                                                                                                                              ;
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Sivas ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}                                                                                                                                              ;
...
text: `Sivas, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/sivas" class="hover:text-slate-900 transition-colors">SİVAS</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/sivas/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{ titleCase(neighData.ilce) }}</NuxtLink>
</nav>
...
<p class="text-slate-500 font-medium uppercase tracking-widest text-sm">{{ titleCase(neighData.ilce) }}, Sivas</p>
</div>
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Sivas {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne aittir.</p>
</div>
...
<p class="text-slate-600">Sivas {{ titleCase(neighData.ilce) }} {{ titleCase(neighData.mahalle) }}'ne ait güncel posta kodu bilgileri aşağıda yer almaktadır. Adres formlarında, kargo gönderilerinde ve resmi işlemlerde hata payını sıfıra indirmek için bu kodu kullanabilirsiniz.</p>
<br>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Sivas ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde ;</p>
...
<NuxtLink to="/sivas" class="mt-6 inline-block text-red-700 underline">Sivas Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
