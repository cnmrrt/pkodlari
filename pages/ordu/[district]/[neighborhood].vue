...
return orduData.value.find(item =>
...
text: `Ordu, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/ordu" class="hover:text-slate-900 transition-colors">ORDU</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/ordu/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Ordu {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Ordu ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/ordu" class="mt-6 inline-block text-red-700 underline">Ordu Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
