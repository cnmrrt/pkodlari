...
const { data: istanbulSileData } = await useAsyncData("istanbul-sile-data", () =>
  $fetch<any[]>("https://pkodlari.com/data/izmir/bornova.json")
);
...
<template>
  <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
    <nav
      ...
      </svg><a href="/izmir" class="hover:text-slate-900 transition-colors">İzmir</a><svg
        xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
        class="lucide lucide-chevron-right-icon w-3 h-3">
        <path d="m9 18 6-6-6-6"></path>
      </svg><a href="/izmir/bornova" class="hover:text-slate-900 transition-colors">{{
        currentDistrictName }}</a></nav>
...
<h2>Bu ilçedeki diğer mahalleler</h2>      <div class="neighborhoods-links-container">
    ...
    <div class="page-text">
      ...
      <p>
        ...
        plaka kodunu ({{ String(neighData.postaKodu).substring(0, 2) }}), son 3 hanesi ise ilçe ve mahalle dağıtım
