...
const { data: gumushaneData } = await useAsyncData('gumushane-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/gumushane.json')
)                                                                                                               ;
...
return gumushaneData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Gümüşhane)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Gümüşhane ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
"acceptedAnswer": {
"@type": "Answer",
"text": `${neighData.value.postaKodu} posta kodu Gümüşhane ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
}
}]
})
}                                                                                                                                            ;
...
text: `Gümüşhane, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/gumushane" class="hover:text-slate-900 transition-colors">GÜMÜŞHANE</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/gumushane/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Gümüşhane {{
...
<p class="text-slate-600">Gümüşhane {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Gümüşhane ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/gumushane" class="mt-6 inline-block text-red-700 underline">Gümüşhane Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
