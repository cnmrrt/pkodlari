...
const { data: manisaData } = await useAsyncData('manisa-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/manisa.json')
)                                                                                                                ;
...
return manisaData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Manisa ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Manisa, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/manisa" class="hover:text-slate-900 transition-colors">MANİSA</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/manisa/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Manisa {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Manisa ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/manisa" class="mt-6 inline-block text-red-700 underline">Manisa Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
