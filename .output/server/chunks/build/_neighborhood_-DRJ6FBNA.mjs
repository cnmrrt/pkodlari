...
import { u as usePageSeo, b as useRequestURL } from './usePageSeo-CgmHQHjs.mjs';
...
const cityName = "Bal\u0131kesir";
const citySlug = "balikesir";
const districtName = "Band\u0131rma";
const districtSlug = "bandirma";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: balikesirBandirmaData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "balikesir-bandirma-data",
      () => $fetch(useRequestURL().origin + "/data/balikesir/bandirma.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!balikesirBandirmaData.value) return null;
      return balikesirBandirmaData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!balikesirBandirmaData.value || !neighData.value) return [];
      return balikesirBandirmaData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/balikesir/bandirma/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-DRJ6FBNA.mjs.map
