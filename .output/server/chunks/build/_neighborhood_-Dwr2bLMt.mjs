...
const districtName = "Burhaniye";
const districtSlug = "burhaniye";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: balikesirBurhaniyeData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "balikesir-burhaniye-data",
      () => $fetch("https://pkodlari.com/data/tr/balikesir/burhaniye.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!balikesirBurhaniyeData.value) return null;
      return balikesirBurhaniyeData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!balikesirBurhaniyeData.value || !neighData.value) return [];
      return balikesirBurhaniyeData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/balikesir/burhaniye/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-Dwr2bLMt.mjs.map
