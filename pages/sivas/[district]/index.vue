...
const pageTitle = computed(() =>
  districtData.value
    ? `Sivas ${titleCase(districtData.value.name)} Posta Kodları`
    : "İlçe Bulunamadı"
);
const pageDesc = computed(() =>
  districtData.value
    ? `Sivas ilinin ${titleCase(
...
<template>
  <div v-if="isValid" class="animate-in fade-in duration-500 max-w-4xl mx-auto">
    <div class="header">
      <div class="districtTitleContainer">
        ...
        <div>
          <h1>
            Sivas {{ titleCase(districtData.name) }} Posta Kodları
          </h1>
          <p>Sivas</p>
        </div>
      </div>
  ...
  <div v-else class="py-32 text-center text-slate-400">
    <p>İlçe bulunamadı.</p>
    <NuxtLink to="/sivas" class="text-slate-900 underline mt-4 inline-block"
      >Şırnak sayfasına dön</NuxtLink
    >
  </div>
</template>
