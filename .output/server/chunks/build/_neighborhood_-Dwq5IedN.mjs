...
const cityName = "Sivas";
const citySlug = "sivas";
const districtName = "Kangal";
const districtSlug = "kangal";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: sivasKangalData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "sivas-kangal-data",
      () => $fetch(useRequestURL().origin + "/data/sivas/kangal.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!sivasKangalData.value) return null;
      return sivasKangalData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!sivasKangalData.value || !neighData.value) return [];
      return sivasKangalData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/sivas/kangal/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-Dwq5IedN.mjs.map
