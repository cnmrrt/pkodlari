...
const { data: batmanData } = await useAsyncData('batman-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/batman.json')
)                                                                                                               ;
...
return batmanData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Batman ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Batman, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/batman" class="hover:text-slate-900 transition-colors">BATMAN</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/batman/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Batman {{
...
<p class="text-slate-600">Posta kodu, bir adresin en spesifik bileşenidir. {{ titleCase(neighData.mahalle) }} için tanımlanan {{ neighData.postaKodu }} numarasını kullanmak şu abatmantajları sağlar:</p>
<ul>
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Batman ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/batman" class="mt-6 inline-block text-red-700 underline">Batman Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
