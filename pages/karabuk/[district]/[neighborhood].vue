...
const { data: karabukData } = await useAsyncData('karabuk-data', () =>
    $fetch<any[]>('https://pkodlari.com/data/tr/karabuk.json')
);
...
const neighData = computed(() => {
    if (!karabukData.value) return null;
    return karabukData.value.find(item =>
...
const pageTitle = computed(() => neighData.value ? `${titleCase(neighData.value.mahalle)} Posta Kodu (${titleCase(neighData.value.ilce)}/Karabük)` : 'Posta Kodu Bulunamadı');
const pageDesc = computed(() => neighData.value ? `Karabük ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
                ...
                children: JSON.stringify({
                    ...
                    "itemListElement": [
                        ...
                        },
                        {
                            ...
                            "position": 2,
                            "name": "Karabük",
                            "item": "https://pkodlari.com/karabuk"
                        },
                        {
                            "@type": "ListItem",
                            "position": 3,
                            "name": titleCase(neighData.value.ilce),
                            "item": `https://pkodlari.com/karabuk/${districtSlug.value}`
                        },
                        {
                            "@type": "ListItem",
                            "position": 4,
                            "name": titleCase(neighData.value.mahalle),
                            "item": `https://pkodlari.com/karabuk/${districtSlug.value}/${neighborhoodSlug.value}`
                        }
                    ]
                })
            };
            ...
            return {
                ...
                children: JSON.stringify({
                    ...
                    "mainEntity": [{
                        ...
                        "acceptedAnswer": {
                            "@type": "Answer",
                            "text": `${neighData.value.postaKodu} posta kodu Karabük ${titleCase(neighData.value.ilce)} ${titleCase(neighData.value.mahalle)}'ne aittir.`
                        }
                    }]
                })
            };
...
const share = () => {
    if (navigator.share && neighData.value) {
        navigator.share({
            ...
            text: `Karabük, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<template>
    <div v-if="isValid" class="main-content-neighborhood animate-in fade-in">
        ...
        <nav class="breadcrumb">
            ...
            <NuxtLink to="/karabuk" class="hover:text-slate-900 transition-colors">KARABÜK</NuxtLink>
            <ChevronRight class="w-3 h-3" />
            <NuxtLink :to="`/karabuk/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
        ...
        <div class="page-text">
            ...
            <p>{{ neighData.postaKodu }} posta kodu Karabük {{
        ...
        <div class="page-text">
            ...
            <p>{{ titleCase(neighData.mahalle) }}, Karabük ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır.
                Posta kodu yapısı incelendiğinde ;</p>
            <ol>
    ...
    <div v-else class="max-w-2xl mx-auto py-12 text-center text-slate-500">
        <div class="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
            ...
            <NuxtLink to="/karabuk" class="mt-6 inline-block text-red-700 underline">Karabük Sayfasına Dön</NuxtLink>
        </div>
    </div>
</template>
