...
const { data: bitlisData } = await useAsyncData('bitlis-data', () =>
$fetch<any[]>('https://words-from-life-5cb26-default-rtdb.firebaseio.com/posta_kodlari_duzenlenmis/bitlis.json')
)                                                                                                               ;
...
return bitlisData.value.find(item =>
...
const pageDesc = computed(() => neighData.value ? `Bitlis ilinin ${titleCase(neighData.value.ilce)} ilçesine bağlı ${titleCase(neighData.value.mahalle)}'nin posta kodunu görmek için tıklayın!` : 'Posta Kodu Rehberi');
...
text: `Bitlis, ${titleCase(neighData.value.ilce)}, ${titleCase(neighData.value.mahalle)} mahallesinin posta kodu: ${neighData.value.postaKodu}`,
...
<NuxtLink to="/bitlis" class="hover:text-slate-900 transition-colors">BİTLİS</NuxtLink>
<ChevronRight class="w-3 h-3" />
<NuxtLink :to="`/bitlis/${districtSlug}`" class="hover:text-slate-900 transition-colors">{{
...
<p class="text-slate-600">{{ neighData.postaKodu }} posta kodu Bitlis {{
...
<p class="text-slate-600">{{ titleCase(neighData.mahalle) }}, Bitlis ilinin {{ titleCase(neighData.ilce) }} ilçesine bağlıdır. Posta kodu yapısı incelendiğinde                                                                                              ;</p>
<ol>
...
<NuxtLink to="/bitlis" class="mt-6 inline-block text-red-700 underline">Bitlis Sayfasına Dön</NuxtLink>
</div>
</div>
</template>
