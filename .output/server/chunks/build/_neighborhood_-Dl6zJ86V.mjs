...
const cityName = "Ad\u0131yaman";
const citySlug = "adiyaman";
const districtName = "Besni";
const districtSlug = "besni";
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data: adiyamanBesniData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "adiyaman-besni-data",
      () => $fetch("https://pkodlari.com/data/adiyaman/besni.json")
    )), __temp = await __temp, __restore(), __temp);
    const neighborhoodSlug = computed(() => route.params.neighborhood);
    const neighData = computed(() => {
      if (!adiyamanBesniData.value) return null;
      return adiyamanBesniData.value.find(
    ...
    const otherNeighborhoods = computed(() => {
      if (!adiyamanBesniData.value || !neighData.value) return [];
      return adiyamanBesniData.value.filter((item) => slugify(item.ilce || districtName) === districtSlug).map((item) => ({
        name: item.mahalle,
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/adiyaman/besni/[neighborhood].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_neighborhood_-Dl6zJ86V.mjs.map
